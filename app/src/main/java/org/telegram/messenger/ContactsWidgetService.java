			...
			if (num >= dids.size()) {
				...
				if (DialogObject.isUserDialog(id)) {
					...
					if (user instanceof TLRPC.TLUser usr) {
						var photoSmall = TLRPCExtensions.getPhotoSmall(usr.photo);

						if (!UserObject.isReplyUser(user) && !UserObject.isUserSelf(user) && photoSmall != null && photoSmall.volumeId != 0 && photoSmall.localId != 0) {
				...
				if (dialog instanceof TLRPC.TLDialog dlg && dlg.unreadCount > 0) {
					String count;
					if (dlg.unreadCount > 99) {
						count = String.format(Locale.getDefault(), "%d+", 99);
					}
					else {
						count = String.format(Locale.getDefault(), "%d", dlg.unreadCount);
					}
