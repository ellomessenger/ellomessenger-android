...
public class SvgHelper {
	...
	public static class SvgDrawable extends Drawable {
		...
		private final Bitmap[] backgroundBitmap = new Bitmap[2];
		private final Canvas[] backgroundCanvas = new Canvas[2];
		private final LinearGradient[] placeholderGradient = new LinearGradient[2];
		private final Matrix[] placeholderMatrix = new Matrix[2];
		...
		public void drawInternal(Canvas canvas, boolean drawInBackground, long time, float x, float y, float w, float h) {
			if (currentColorValue != 0) {
				setupGradient(currentColorValue, colorAlpha, drawInBackground);
			}
			...
			if (placeholderGradient != null) {
				if (drawInBackground) {
					...
					if (shiftRunnable == null || shiftDrawable.get() == this) {
						...
						// ensure that this is not the situation
						// when both values are zeros -
						// in that case this was becoming an infinite loop
						if (!(totalTranslation == 0.0 && gradientWidth == 0.0)) {
							while (totalTranslation >= gradientWidth / 2) {
		...
		public void setupGradient(int color, float alpha, boolean drawInBackground) {
			int index = drawInBackground ? 1 : 0;
	...
	private static class SVGHandler extends DefaultHandler {
		...
		private final Integer paintColor;
		private float globalScale = 1f;

		boolean pushed = false;

		private final HashMap<String, StyleSet> globalStyles = new HashMap<>();
