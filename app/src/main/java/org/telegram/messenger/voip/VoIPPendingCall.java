...
public final class VoIPPendingCall {
	...
	private boolean onConnectionStateUpdated(boolean force) {
		if (!released && (force || isConnected(accountInstance) || isAirplaneMode())) {
			...
			final User user = messagesController.getUser(userId);
			if (user != null) {
				final TLUserFull userFull = messagesController.getUserFull(user.id);
				VoIPHelper.startCall(user, video, userFull != null && userFull.videoCallsAvailable, activity, userFull, accountInstance);
			}
