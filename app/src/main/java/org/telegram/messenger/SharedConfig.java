...
public class SharedConfig {
	public final static int PASSCODE_TYPE_PIN = 0, PASSCODE_TYPE_PASSWORD = 1;

	@Retention(RetentionPolicy.SOURCE)
	@IntDef({PASSCODE_TYPE_PIN, PASSCODE_TYPE_PASSWORD})
	public @interface PasscodeType {
	}

	public static final int MAP_PREVIEW_PROVIDER_ELLO = 0;
	public static final int MAP_PREVIEW_PROVIDER_GOOGLE = 1;
	public static final int MAP_PREVIEW_PROVIDER_NONE = 2;
	public static final int MAP_PREVIEW_PROVIDER_YANDEX = 3;
	...
	public static int mapPreviewType = SharedConfig.MAP_PREVIEW_PROVIDER_GOOGLE;
	...
	public static void loadConfig() {
		synchronized (sync) {
			...
			if (saveToGalleryLegacy && BuildVars.NO_SCOPED_STORAGE) {
				...
				preferences.edit().remove("save_gallery").putInt("save_gallery_flags", saveToGalleryFlags).commit();
			}
			else {
			...
			mapPreviewType = preferences.getInt("mapPreviewType", MAP_PREVIEW_PROVIDER_NONE);
			...
			bubbleRadius = preferences.getInt("bubbleRadius", 12);
			...
			streamAllVideo = preferences.getBoolean("streamAllVideo", BuildConfig.DEBUG);
			streamMkv = preferences.getBoolean("streamMkv", false);
			suggestStickers = preferences.getInt("suggestStickers", 0);
			suggestAnimatedEmoji = preferences.getBoolean("suggestAnimatedEmoji", true);
			sortContactsByName = preferences.getBoolean("sortContactsByName", true);
	...
	public static void checkSaveToGalleryFiles() {
		Utilities.globalQueue.postRunnable(() -> {
			try {
				File telegramPath = new File(Environment.getExternalStorageDirectory(), "Ello");
	...
	public static int getChatSwipeAction(int currentAccount) {
		// MARK: uncomment to enable other actions
...
//		return SwipeGestureSettingsView.SWIPE_GESTURE_ARCHIVE;
	...
	public static void updateChatListSwipeSetting(int newAction) {
		...
		SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);
		preferences.edit().putInt("ChatSwipeAction", chatSwipeAction).commit();
	}

	public static void updateMessageSeenHintCount(int count) {
		messageSeenHintCount = count;
		SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);
		preferences.edit().putInt("messageSeenCount", messageSeenHintCount).commit();
	}

	public static void updateEmojiInteractionsHintCount(int count) {
		emojiInteractionsHintCount = count;
		SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);
		preferences.edit().putInt("emojiInteractionsHintCount", emojiInteractionsHintCount).commit();
	}

	public static void updateDayNightThemeSwitchHintCount(int count) {
		dayNightThemeSwitchHintCount = count;
		SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);
		preferences.edit().putInt("dayNightThemeSwitchHintCount", dayNightThemeSwitchHintCount).commit();
	}
	...
	@Retention(RetentionPolicy.SOURCE)
	@IntDef({PERFORMANCE_CLASS_LOW, PERFORMANCE_CLASS_AVERAGE, PERFORMANCE_CLASS_HIGH})
	...
	public static void setMediaColumnsCount(int count) {
		if (mediaColumnsCount != count) {
			...
			ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE).edit().putInt("mediaColumnsCount", mediaColumnsCount).commit();
		}
	}
	...
	public static void setFastScrollHintCount(int count) {
		if (fastScrollHintCount != count) {
			...
			ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE).edit().putInt("fastScrollHintCount", fastScrollHintCount).commit();
		}
	}

	public static void setDontAskManageStorage(boolean b) {
		dontAskManageStorage = b;
		ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE).edit().putBoolean("dontAskManageStorage", dontAskManageStorage).commit();
	}
	...
	public static class BackgroundActivityPrefs {
		...
		public static void setLastCheckedBackgroundActivity(long l) {
			prefs.edit().putLong("last_checked", l).commit();
		}
	}
