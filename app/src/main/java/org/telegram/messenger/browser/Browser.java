...
public class Browser {
	...
	public static void openUrl(@Nullable Context context, Uri uri) {
		openUrl(context, uri, true);
	}

	public static void openUrl(@Nullable Context context, String url, boolean allowCustom) {
	...
	public static boolean urlMustNotHaveConfirmation(String url) {
		return (isTelegraphUrl(url, false, true) || url.matches("^(https://)?" + BuildConfig.MESSENGER_ENDPOINT + "/iv\\??.*") || // example.com/iv?
				url.matches("^(https://)?" + BuildConfig.MESSENGER_ENDPOINT + "/(blog|tour)/?.*") // example.com/blog, example.com/tour
		);
	}
	...
	public static void openUrl(@Nullable final Context context, @Nullable Uri uri, final boolean allowCustom, boolean tryTelegraph) {
		...
		if (tryTelegraph) {
			try {
				...
				if (isTelegraphUrl(host, true) || uri.toString().toLowerCase().contains("ello.team/faq") || uri.toString().toLowerCase().contains("ello.team/privacy")) {
		...
		try {
			...
			if (allowCustom && SharedConfig.customTabs && !internalUri && !scheme.equals("tel")) {
				...
				if (forceBrowser[0] || allActivities == null || allActivities.isEmpty()) {
					...
					builder.addMenuItem(context.getString(R.string.CopyLink), copy);

					builder.setToolbarColor(Theme.getColor(Theme.key_actionBarBrowser));
					builder.setShowTitle(true);
					builder.setActionButton(BitmapFactory.decodeResource(context.getResources(), R.drawable.msg_filled_shareout), context.getString(R.string.ShareFile), PendingIntent.getBroadcast(ApplicationLoader.applicationContext, 0, share, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE), true);
	...
	public static boolean isPassportUrl(String url) {
		...
		try {
			url = url.toLowerCase();
			if (url.startsWith("elloapp:passport") || url.startsWith("elloapp://passport") || url.startsWith("elloapp:secureid") || url.contains("resolve") && url.contains("domain=elloapppassport")) {
				return true;
			}
		}
		catch (Throwable ignore) {
	...
	public static boolean isInternalUri(Uri uri, boolean all, boolean[] forceBrowser) {
		...
		if ("ton".equals(uri.getScheme())) {
		...
		else if ("elloapp".equals(uri.getScheme())) {
			return true;
		}
		else if (ApplicationLoader.applicationContext.getString(R.string.domain).equals(host)) {
			...
			return host.endsWith("ello.team");
		}
