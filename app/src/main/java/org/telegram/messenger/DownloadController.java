...
public class DownloadController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
	...
	public static class Preset {
		...
		public void set(TLRPC.TLAutoDownloadSettings settings) {
			preloadMusic = settings.audioPreloadNext;
			preloadVideo = settings.videoPreloadLarge;
			lessCallData = settings.phonecallsLessData;
			maxVideoBitrate = settings.videoUploadMaxbitrate;

			sizes[PRESET_SIZE_NUM_PHOTO] = Math.max(500 * 1024, settings.photoSizeMax);
			sizes[PRESET_SIZE_NUM_VIDEO] = Math.max(500 * 1024, settings.videoSizeMax);
			sizes[PRESET_SIZE_NUM_DOCUMENT] = Math.max(500 * 1024, settings.fileSizeMax);

			for (int a = 0; a < mask.length; a++) {
				if (settings.photoSizeMax != 0 && !settings.disabled) {
					mask[a] |= AUTODOWNLOAD_TYPE_PHOTO;
				}
				else {
					mask[a] &= ~AUTODOWNLOAD_TYPE_PHOTO;
				}
				if (settings.videoSizeMax != 0 && !settings.disabled) {
					mask[a] |= AUTODOWNLOAD_TYPE_VIDEO;
				}
				else {
					mask[a] &= ~AUTODOWNLOAD_TYPE_VIDEO;
				}
				if (settings.fileSizeMax != 0 && !settings.disabled) {
		...
		public String toString() {
			return mask[0] + "_" + mask[1] + "_" + mask[2] + "_" + mask[3] + "_" + sizes[PRESET_SIZE_NUM_PHOTO] + "_" + sizes[PRESET_SIZE_NUM_VIDEO] + "_" + sizes[PRESET_SIZE_NUM_DOCUMENT] + "_" + sizes[PRESET_SIZE_NUM_AUDIO] + "_" + (preloadVideo ? 1 : 0) + "_" + (preloadMusic ? 1 : 0) + "_" + (enabled ? 1 : 0) + "_" + (lessCallData ? 1 : 0) + "_" + maxVideoBitrate;
		}

		public boolean equals(Preset obj) {
			return mask[0] == obj.mask[0] && mask[1] == obj.mask[1] && mask[2] == obj.mask[2] && mask[3] == obj.mask[3] && sizes[0] == obj.sizes[0] && sizes[1] == obj.sizes[1] && sizes[2] == obj.sizes[2] && sizes[3] == obj.sizes[3] && preloadVideo == obj.preloadVideo && preloadMusic == obj.preloadMusic && maxVideoBitrate == obj.maxVideoBitrate;
		}
	...
	public void checkAutodownloadSettings() {
		...
		if ((currentMask & AUTODOWNLOAD_TYPE_PHOTO) != 0) {
			...
			for (int a = 0; a < photoDownloadQueue.size(); a++) {
				...
				if (downloadObject.object instanceof TLRPC.TLPhoto photo) {
	...
	public int canDownloadMedia(TLRPC.Message message) {
		...
		int index;
		TLRPC.Peer peer = message.peerId;
		if (peer != null) {
			if (TLRPCExtensions.getUserId(peer) != 0) {
				if (getContactsController().contactsDict.containsKey(TLRPCExtensions.getUserId(peer))) {
					index = 0;
				}
				else {
					index = 1;
				...
				if (message.fromId instanceof TLRPC.TLPeerUser && getContactsController().contactsDict.containsKey(TLRPCExtensions.getUserId(message.fromId))) {
					index = 0;
				}
				else {
					index = 2;
				...
				TLRPC.Chat chat = TLRPCExtensions.getChannelId(message.peerId) != 0 ? getMessagesController().getChat(TLRPCExtensions.getChannelId(message.peerId)) : null;
	...
	public void savePresetToServer(int type) {
		var req = new TLRPC.TLAccountSaveAutoDownloadSettings();
		Preset preset;
		boolean enabled;
		...
		req.settings = new TLRPC.TLAutoDownloadSettings();
		req.settings.audioPreloadNext = preset.preloadMusic;
		req.settings.videoPreloadLarge = preset.preloadVideo;
		req.settings.phonecallsLessData = preset.lessCallData;
		req.settings.videoUploadMaxbitrate = preset.maxVideoBitrate;
		...
		req.settings.photoSizeMax = photo ? (int)preset.sizes[PRESET_SIZE_NUM_PHOTO] : 0;
		req.settings.videoSizeMax = video ? preset.sizes[PRESET_SIZE_NUM_VIDEO] : 0;
		req.settings.fileSizeMax = document ? preset.sizes[PRESET_SIZE_NUM_DOCUMENT] : 0;
	...
	protected void cancelDownloading(ArrayList<Pair<Long, Integer>> arrayList) {
		for (int a = 0, N = arrayList.size(); a < N; a++) {
			...
			if (downloadObject.object instanceof TLRPC.Document document) {
				getFileLoader().cancelLoadFile(document);
			}
			else if (downloadObject.object instanceof TLRPC.TLPhoto photo) {
				TLRPC.PhotoSize photoSize = FileLoader.getClosestPhotoSizeWithSize(photo.sizes, AndroidUtilities.getPhotoSize());
	...
	protected void processDownloadObjects(int type, ArrayList<DownloadObject> objects) {
		...
		for (int a = 0; a < objects.size(); a++) {
			...
			else if (downloadObject.object instanceof TLRPC.Document document) {
	...
	public void didReceivedNotification(int id, int account, Object... args) {
		if (id == NotificationCenter.fileLoadFailed || id == NotificationCenter.httpFileDidFailedLoad) {
			...
			processLaterArrays();
			try {
				List<SendMessagesHelper.DelayedMessage> delayedMessages = getSendMessagesHelper().getDelayedMessages(fileName);
				if (delayedMessages != null) {
					for (int a = 0; a < delayedMessages.size(); a++) {
						SendMessagesHelper.DelayedMessage delayedMessage = delayedMessages.get(a);
						if (delayedMessage.encryptedChat == null) {
							long dialogId = delayedMessage.getPeer();
	...
	public void startDownloadFile(MessageObject parentObject) {
			...
			if (removed) {
				...
				getMessagesStorage().getStorageQueue().postRunnable(() -> {
					try {
						...
						String req = String.format(Locale.ENGLISH, "UPDATE downloading_documents SET state = 1, date = %d WHERE hash = %d AND id = %d", System.currentTimeMillis(), dcId, parentObject.getDocument().id);
	...
	Runnable clearUnviewedDownloadsRunnable = () -> {
	...
	private void putToUnviewedDownloads(MessageObject parentObject) {
		...
		AndroidUtilities.cancelRunOnUIThread(clearUnviewedDownloadsRunnable);
		AndroidUtilities.runOnUIThread(clearUnviewedDownloadsRunnable, 60000);
	}
		...
		getMessagesStorage().getStorageQueue().postRunnable(() -> {
			...
			try {
				...
				while (cursor2.next()) {
					...
					if (data != null) {
						var message = Message.deserialize(data, data.readInt32(false), false);
			...
			getFileLoader().checkMediaExistence(downloadingMessages);
			getFileLoader().checkMediaExistence(recentlyDownloadedMessages);
	...
	public void deleteRecentFiles(ArrayList<MessageObject> messageObjects) {
		for (int i = 0; i < messageObjects.size(); i++) {
			...
			FileLoader.getInstance(currentAccount).cancelLoadFile(messageObjects.get(i).getDocument());
		}
		getNotificationCenter().postNotificationName(NotificationCenter.onDownloadingFilesChanged);
		getMessagesStorage().getStorageQueue().postRunnable(() -> {
			try {
				...
				for (int i = 0; i < messageObjects.size(); i++) {
					...
					if (messageObjects.get(i).getDocument() instanceof TLRPC.TLDocument doc) {
						dcId = doc.dcId;
						documentId = doc.id;
					}

					state.bindInteger(1, dcId);
					state.bindLong(2, documentId);
					state.step();
