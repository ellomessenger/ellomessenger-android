...
public class NotificationBadge {
			...
			try {
				...
				if (cursor != null) {
					...
					while (cursor.moveToNext()) {
						...
						int colIndex = cursor.getColumnIndex("class");

						if (colIndex != -1) {
							if (entryActivityName.equals(cursor.getString(colIndex))) {
	...
	public static class XiaomiHomeBadger implements Badger {
		...
		@Override
		public void executeBadge(int badgeCount) {
			String badgeCountStr = String.valueOf(badgeCount == 0 ? "" : badgeCount);
			...
			try {
				...
				field.set(miuiNotification, badgeCountStr);
			}
			catch (Throwable e) {
				...
				localIntent.putExtra(EXTRA_UPDATE_APP_MSG_TEXT, badgeCountStr);

				if (canResolveBroadcast(localIntent)) {
					AndroidUtilities.runOnUIThread(() -> ApplicationLoader.applicationContext.sendBroadcast(localIntent));
				}
			}
		}
	...
	public static void applyCount(int badgeCount) {
		try {
