...
import org.telegram.tgnet.TLRPC.TL_biz_invokeBizDataRaw
...
object ElloRpc {
	...
	data class AuthUser(
		...
		fun toTLRPCUser(): User {
			...
			user.mutual_contact = isMutualContact
			user.access_hash = accessHash.value
			user.status = TLRPC.TL_userStatusRecently()
			return user
		}
	}
	...
	data class UserWallet(
		...
		companion object {
			fun fromMap(map: Map<String, Any>): UserWallet? {
				return UserWallet(
						id = (map["id"] as? Number ?: return null).toLong(),
						name = map["asset_name"] as? String ?: return null,
						symbol = (map["asset_symbol"] as? String)?.lowercase() ?: return null,
						type = map["type"] as? String ?: return null,
						amount = (map["amount"] as? Number ?: return null).toFloat(),
				)
			}
		}
	}
	...
	data class TransactionHistoryEntry(
		...
		enum class PeerType(val value: Int) {
			CHANNEL_SUBSCRIPTION(0), COURSE_CHANNEL(1), TOPUP_OR_WITHDRAW(2), SINGLE_MEDIA_PURCHASE(3), AI_BOT_SUBSCRIPTION(4), AI_BOT_PURCHASE(5), TRANSFER_BETWEEN_WALLETS(6), AI_TEXT_PACK(7), AI_IMAGE_PACK(8), AI_TEXT_SUBSCRIPTION(9), AI_IMAGE_SUBSCRIPTION(10), REFERRAL_COMMISSION(11), REFERRAL_BONUS(12), AI_IMAGE_TEXT_PACK(13);
			...
			val isInternal: Boolean
				get() = when (this) {
					TRANSFER_BETWEEN_WALLETS, TOPUP_OR_WITHDRAW, AI_BOT_SUBSCRIPTION, AI_BOT_PURCHASE, AI_TEXT_PACK, AI_IMAGE_PACK, AI_TEXT_SUBSCRIPTION, AI_IMAGE_SUBSCRIPTION, AI_IMAGE_TEXT_PACK -> true
					else -> false
				}
		}
...
//				} ?: 0f
	...
	data class SubscriptionItem(
			...
			@field:SerializedName("peer_id") val channelId: Long,
			@field:SerializedName("peer_type") val type: Int,
			@field:SerializedName("amount") val amount: Float,
			@field:SerializedName("currency") val currency: String,
	...
	@Keep
	data class SubscriptionInfoAiBot(
			@field:SerializedName("easy_mode") val isEasyMode: Boolean,
			@field:SerializedName("text_total") val textTotal: Int,
			@field:SerializedName("text_expire_at") val textExpireAt: Long,
			@field:SerializedName("text_sub_active") val isTextSubscriptionActive: Boolean,
			@field:SerializedName("img_total") val imgTotal: Int,
...
//	}
	...
	fun forgotPasswordRequest(email: String): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(authService, 100400, mapOf("email" to email))
		return createRpcRequest(elloRequest)
	}

	fun newPasswordRequest(email: String, code: String, password: String): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(authService, 100500, mapOf("email" to email, "code" to code, "new_pass" to password))
		return createRpcRequest(elloRequest)
	}

	fun verifySignupRequest(username: String, code: String): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(authService, 100800, mapOf("username_or_email" to username, "code" to code))
		return createRpcRequest(elloRequest)
	}

	fun verifyRequest(username: String, code: String): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(authService, 100300, mapOf("username_or_email" to username, "code" to code))
		return createRpcRequest(elloRequest)
	}

	fun verifyLoginRequest(username: String, code: String): TL_biz_invokeBizDataRaw {
	...
	fun verificationCodeRequest(email: String, password: String): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(profileService, 100500, mapOf("email" to email, "password" to password))
		return createRpcRequest(elloRequest)
	}

	fun changeEmailRequest(newEmail: String, code: String): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(profileService, 100100, mapOf("new_email" to newEmail, "code" to code))
		return createRpcRequest(elloRequest)
	}

	fun withdrawSendApproveCode(): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(walletService, 102100, mapOf())
		return createRpcRequest(elloRequest)
	}

	fun deleteAccountVerificationCodeRequest(email: String, password: String): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(profileService, 100800, mapOf("email" to email, "password" to password))
		return createRpcRequest(elloRequest)
	}

	fun deleteAccount(code: String): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(profileService, 100400, mapOf("code" to code, "reason" to "So Long, and Thanks for All the Fish"))
		return createRpcRequest(elloRequest)
	}

	fun changeProfileRequest(firstName: String?, lastName: String?, username: String, bio: String, gender: String?, birthday: String?, country: String?): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["username"] = username
		data["gender"] = gender ?: ""
	...
	fun channelCategoriesRequest(): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(channelsService, 100100, mapOf())
		return createRpcRequest(elloRequest)
	}

	fun channelMusicGenresRequest(): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(channelsService, 100200, mapOf())
		return createRpcRequest(elloRequest)
	}

	fun deleteAccountRequest(): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(profileService, 100400, mapOf())
		return createRpcRequest(elloRequest)
	}

	fun feedSettingsRequest(): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(feedService, 100100, mapOf())
		return createRpcRequest(elloRequest)
	}

	fun saveFeedSettingsRequest(hidden: LongArray?, pinned: LongArray?, showRecommended: Boolean?, showSubscriptionsOnly: Boolean?, showAdult: Boolean?): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
	...
	fun resendConfirmationCodeRequest(email: String): TL_biz_invokeBizDataRaw {
		val data = mapOf("username_or_email" to email)
		val elloRequest = ElloRequest(authService, 100600, data)
		return createRpcRequest(elloRequest)
	}

	fun availableWalletAssetsRequest(): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(walletService, 100300, mapOf())
		return createRpcRequest(elloRequest)
	}

	fun loadWalletRequest(assetId: Int): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(walletService, 100200, mapOf("asset_id" to assetId))
		return createRpcRequest(elloRequest)
	}

	fun createWalletRequest(assetId: Int): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(walletService, 100100, mapOf("asset_id" to assetId))
		return createRpcRequest(elloRequest)
	}

	fun loadEarningsRequest(walletId: Long): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(walletService, 101100, mapOf("wallet_id" to walletId))
		return createRpcRequest(elloRequest)
	}

//	fun stripePaymentRequest(assetId: Int, walletId: Long, currency: String, message: String?, cardNumber: String, expirationMonth: Int, expirationYear: Int, cvc: Int, amount: Float): TL_biz_invokeBizDataRaw {
//		val data = mutableMapOf<String, Any>()
...
//	}
	...
	fun paypalPaymentRequest(assetId: Int, walletId: Long, currency: String, message: String?, amount: Float?, coin: Float?): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["asset_id"] = assetId
		data["wallet_id"] = walletId
		data["currency"] = currency
	...
	fun stripePaymentRequest(assetId: Int, walletId: Long, currency: String, message: String?, amount: Float?, coin: Float?): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["asset_id"] = assetId
		data["wallet_id"] = walletId
		data["currency"] = currency
	...
	fun transactionsStatsRequest(walletId: Long): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["period"] = "day"
		data["wallet_id"] = walletId
	 ...
	 */
	@JvmOverloads
	fun subscribeRequest(peerId: Long, type: Int = 4): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["peer_id"] = peerId
		data["peer_type"] = type
	 ...
	 * @param type 2 - user, 3 - chat, 4 - channel (default value 4)
	 */
	fun unsubscribeRequest(peerId: Long, type: Int = 4): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["peer_id"] = peerId
		data["peer_type"] = type
	 ...
	 */
	fun getTransactionsHistoryRequest(assetId: Int, walletId: Long, paymentType: String, query: String?, dateFrom: String?, dateTo: String?, limit: Int, offset: Int): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["asset_id"] = assetId
		data["wallet_id"] = walletId
	...
	fun getWalletPaymentById(walletId: Long, paymentId: Long): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["wallet_id"] = walletId
	 ...
	 * @param offset offset
	 */
	fun getTransferStatisticGraphic(walletId: Long, period: String, type: String, limit: Int, offset: Int): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["wallet_id"] = walletId
		data["period"] = period
		data["type"] = type
	...
	fun getLastMonthActivityGraphics(walletId: Long, limit: Int, page: Int): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["wallet_id"] = walletId
		data["limit"] = limit
		data["page"] = page
	...
	fun getCalculateTransferFee(fromWalletId: Long, toWalletId: Long, currency: String? = null, message: String? = null, amount: Float): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
	 ...
	 * @param bankWithdrawRequisitesId id of previously saved bank requisites (optional)
	 */
	fun getWithdrawApprovePayment(walletId: Long, paymentId: String, approveCode: String?, paypalEmail: String?, bankWithdrawRequisitesId: Long?): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["wallet_id"] = walletId
		data["payment_id"] = paymentId
	...
	fun stopChatBot(): TL_biz_invokeBizDataRaw {
	 ...
	 */
	fun subscribeChatBotRequest(@androidx.annotation.IntRange(from = 1, to = 4) subType: Int, quantity: Int = 0) = createRpcRequest(ElloRequest(aiGenCustomizeService, 100500, mapOf("sub_type" to subType, "quantity" to quantity)))
	 ...
	 */
	fun transferBetweenWalletsRequest(from: Long, to: Long, amount: Float, currency: String = WalletHelper.DEFAULT_CURRENCY): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
	...
	fun generateShareHash(peerId: Long, peerType: Int, msgId: Int): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["peer_id"] = peerId
	...
	fun userCheckHasDialog(userId: Long): TL_biz_invokeBizDataRaw {
		val elloRequest = ElloRequest(profileService, 100900, mapOf("user_id" to userId))
		return createRpcRequest(elloRequest)
	}

	fun getLikesForMessage(messageId: Int, userId: Long, channelId: Long): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["user_id"] = userId
	...
	fun likeMessage(messageId: Int, userId: Long, channelId: Long): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["user_id"] = userId
		data["msg_id"] = messageId
		data["fromId"] = mapOf("peer_type" to 4, "peer_id" to channelId)
	...
	fun revokeLikeFromMessage(messageId: Int, userId: Long, channelId: Long): TL_biz_invokeBizDataRaw {
		val data = mutableMapOf<String, Any>()
		data["user_id"] = userId
		data["msg_id"] = messageId
		data["fromId"] = mapOf("peer_type" to 4, "peer_id" to channelId)
	...
	fun accountDeletionLeftoversRequest(): TL_biz_invokeBizDataRaw {
		return createRpcRequest(ElloRequest(profileService, 101000, mapOf()))
	}

	private fun createRpcRequest(request: ElloRequest): TL_biz_invokeBizDataRaw {
		val req = TL_biz_invokeBizDataRaw()
		req.biz_data = TL_biz_dataRaw()
		req.biz_data.data = gson.toJson(request).toByteArray(StandardCharsets.UTF_8)
		return req
	}

	fun TL_biz_dataRaw.readString(): String? {
		return try {
	...
	inline fun <reified T> TL_biz_dataRaw.readData(): T? {
