...
class TL_messageEntityMentionName_layer131 : TL_messageEntityMentionName() {
	override fun equals(other: Any?): Boolean {
		if (other !is TL_messageEntityMentionName_layer131) {
			return false
		}
	...
	override fun readParams(stream: AbstractSerializedData?, exception: Boolean) {
		...
		userId = stream.readInt32(exception).toLong()
	}
	...
	override fun serializeToStream(stream: AbstractSerializedData?) {
		...
		stream.writeInt32(userId.toInt())
	}
	...
	companion object {
		const val CONSTRUCTOR = 0x352dca58
	}
}
