...
class TL_inputMessageEntityMentionName : MessageEntity() {
	@JvmField
	var userId: InputUser? = null

	override fun equals(other: Any?): Boolean {
		if (other !is TL_inputMessageEntityMentionName) {
			return false
		}
		...
		return offset == other.offset && length == other.length && userId == other.userId
	}
	...
	override fun readParams(stream: AbstractSerializedData?, exception: Boolean) {
		...
		userId = InputUser.TLdeserialize(stream, stream.readInt32(exception), exception)
	}
	...
	override fun hashCode(): Int {
		return Objects.hash(CONSTRUCTOR, offset, length, userId)
	}

	companion object {
		const val CONSTRUCTOR = 0x208e68c9
	}
}
