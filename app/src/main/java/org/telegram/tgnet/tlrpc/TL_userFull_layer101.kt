...
class TL_userFull_layer101 : TL_userFull() {
	override fun readParams(stream: AbstractSerializedData?, exception: Boolean) {
		...
		link = TLRPC.TL_contacts_link_layer101.TLdeserialize(stream, stream.readInt32(exception), exception)
	...
	companion object {
		const val constructor = 0x745559cc
	}
}
