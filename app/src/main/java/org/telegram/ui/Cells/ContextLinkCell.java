...
public class ContextLinkCell extends FrameLayout implements DownloadController.FileDownloadProgressListener {
	...
	private final int titleY = AndroidUtilities.dp(7);
	private StaticLayout titleLayout;

	private final int descriptionY = AndroidUtilities.dp(27);
	...
	public ContextLinkCell(Context context, boolean needsCheckBox) {
		...
		if (needsCheckBox) {
			...
			backgroundPaint.setColor(context.getColor(R.color.light_background));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (documentAttach != null || currentPhotoObject != null || webFile != null || urlLocation != null) {
			...
			if (documentAttachType == DOCUMENT_ATTACH_TYPE_GIF) {
				...
				if (currentPhotoObject != null) {
					SvgHelper.SvgDrawable svgThumb = DocumentObject.getSvgThumb(documentAttach, ResourcesCompat.getColor(getContext().getResources(), R.color.light_background, null), 1.0f);
		...
		if (mediaWebpage) {
			...
			linkImageView.setImageCoordinates(0, 0, width, height);
		}
		else {
			int height = 0;
			...
			linkImageView.setImageCoordinates(x, AndroidUtilities.dp(8), maxPhotoWidth, maxPhotoWidth);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (descriptionLayout != null) {
			Theme.chat_contextResult_descriptionTextPaint.setColor(getContext().getColor(R.color.dark_gray));
			canvas.save();
		...
		if (linkLayout != null) {
			Theme.chat_contextResult_descriptionTextPaint.setColor(getContext().getColor(R.color.brand));
			canvas.save();
		...
		if (!mediaWebpage) {
			...
			if (documentAttachType == DOCUMENT_ATTACH_TYPE_AUDIO || documentAttachType == DOCUMENT_ATTACH_TYPE_MUSIC) {
				radialProgress.setProgressColor(getContext().getColor(R.color.brand));
	...
	private int getIconForCurrentState() {
		if (documentAttachType == DOCUMENT_ATTACH_TYPE_AUDIO || documentAttachType == DOCUMENT_ATTACH_TYPE_MUSIC) {
			radialProgress.setColors(getContext().getColor(R.color.brand), getContext().getColor(R.color.white), getContext().getColor(R.color.brand), getContext().getColor(R.color.white));
			if (buttonState == 1) {
			...
			Utilities.searchQueue.postRunnable(() -> {
				...
				if (documentAttachType == DOCUMENT_ATTACH_TYPE_MUSIC || documentAttachType == DOCUMENT_ATTACH_TYPE_AUDIO) {
					...
					if (inlineResult != null) {
						...
						else if (inlineResult.photo instanceof TL_photo) {
	...
	public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
		...
		switch (documentAttachType) {
			case DOCUMENT_ATTACH_TYPE_DOCUMENT:
				sbuf.append(getContext().getString(R.string.AttachDocument));
				break;
			case DOCUMENT_ATTACH_TYPE_GIF:
				sbuf.append(getContext().getString(R.string.AttachGif));
				break;
			case DOCUMENT_ATTACH_TYPE_AUDIO:
				sbuf.append(getContext().getString(R.string.AttachAudio));
				break;
			case DOCUMENT_ATTACH_TYPE_VIDEO:
				sbuf.append(getContext().getString(R.string.AttachVideo));
				break;
			case DOCUMENT_ATTACH_TYPE_MUSIC:
				sbuf.append(getContext().getString(R.string.AttachMusic));
				break;
			case DOCUMENT_ATTACH_TYPE_STICKER:
				sbuf.append(getContext().getString(R.string.AttachSticker));
				break;
			case DOCUMENT_ATTACH_TYPE_PHOTO:
				sbuf.append(getContext().getString(R.string.AttachPhoto));
				break;
			case DOCUMENT_ATTACH_TYPE_GEO:
				sbuf.append(getContext().getString(R.string.AttachLocation));
				break;
		}
	...
	public final Property<ContextLinkCell, Float> IMAGE_SCALE = new AnimationProperties.FloatProperty<>("animationValue") {
		@Override
	...
	public void setChecked(boolean checked, boolean animated) {
		...
		if (animated) {
			...
			animator.playTogether(ObjectAnimator.ofFloat(this, IMAGE_SCALE, checked ? 0.81f : 1.0f));
