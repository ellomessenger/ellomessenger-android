...
public class GroupCallUserCell extends FrameLayout {
	...
	private int grayIconColor = ResourcesCompat.getColor(getContext().getResources(), R.color.dark_gray, null);
	...
	public void setProgressToAvatarPreview(float progressToAvatarPreview) {
		...
		if (isSelfUser() && progressToAvatarPreview > 0) {
			...
			for (SimpleTextView simpleTextView : statusTextView) {
				if (!TextUtils.isEmpty(statusTextView[4].getText()) && statusTextView[4].getLineCount() > 1) {
					simpleTextView.setFullLayoutAdditionalWidth(AndroidUtilities.dp(92), LocaleController.isRTL ? AndroidUtilities.dp(48) : AndroidUtilities.dp(53));
					simpleTextView.setFullAlpha(progressToAvatarPreview);
					simpleTextView.setTranslationX(0);
					simpleTextView.invalidate();
				}
				else {
					simpleTextView.setTranslationX((LocaleController.isRTL ? AndroidUtilities.dp(53) : -AndroidUtilities.dp(53)) * progressToAvatarPreview);
					simpleTextView.setFullLayoutAdditionalWidth(0, 0);
				}
			}
		}
	...
	public void setDrawAvatar(boolean draw) {
		if (avatarImageView.imageReceiver.getVisible() != draw) {
			avatarImageView.imageReceiver.setVisible(draw, true);
		}
	}
	...
	public GroupCallUserCell(Context context) {
		...
		dividerPaint.setColor(context.getColor(R.color.divider));
		...
		avatarProgressView = new RadialProgressView(context) {
			...
			protected void onDraw(Canvas canvas) {
				...
				avatarProgressView.setProgressColor(ColorUtils.setAlphaComponent(0xffffffff, (int)(255 * avatarImageView.imageReceiver.getCurrentAlpha() * avatarImageView.getAlpha())));
				super.onDraw(canvas);
			}
		};
		...
		for (int a = 0; a < statusTextView.length; a++) {
			...
			statusTextView[a] = new SimpleTextView(context) {
				...
				public void setFullAlpha(float value) {
					...
					for (SimpleTextView simpleTextView : statusTextView) {
						simpleTextView.setAlpha(simpleTextView.getAlpha());
					}
				}
			};
		...
		muteButton.setAnimation(muteDrawable);
		RippleDrawable rippleDrawable = (RippleDrawable)Theme.createSelectorDrawable(grayIconColor & 0x24ffffff);
	...
	public void setData(AccountInstance account, TLRPC.TL_groupCallParticipant groupCallParticipant, ChatObject.Call call, long self, TLRPC.FileLocation uploadingAvatar, boolean animated) {
		...
		if (id > 0) {
			...
			currentChat = account.getMessagesController().getChat(-id);
			currentUser = null;
	...
	public void setGrayIconColor(int color, int value) {
	...
	private void applyParticipantChanges(boolean animated, boolean internal) {
		...
		if (!isSelfUser()) {
			statusTextView[4].setTextColor(grayIconColor);
		}
