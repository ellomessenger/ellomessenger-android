...
public abstract class TextSelectionHelper<Cell extends TextSelectionHelper.SelectableView> {
	...
	private final RectF endArea = new RectF();
	...
	public interface OnTranslateListener {
		void run(CharSequence text, String fromLang, String toLang, Runnable onAlertDismiss);
	}
	...
	private void showMagnifier(int x) {
		if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.P) {
			...
			magnifier.show(magnifierXanimated, magnifierYanimated + lineHeight * 1.5f + AndroidUtilities.dp(8));
			magnifier.update();
		}
	}
	...
	public class TextSelectionOverlay extends View {
		...
		public boolean checkOnTap(MotionEvent event) {
			if (!isSelectionMode() || movingHandle) {
				return false;
			}
			switch (event.getAction()) {
				case MotionEvent.ACTION_DOWN:
		...
		protected void onDraw(Canvas canvas) {
			...
			if (selectedView != null) {
				...
				if (msg != null && msg.isOutOwner()) {
					handleViewPaint.setColor(getContext().getColor(R.color.white));
				}
				else {
					handleViewPaint.setColor(getContext().getColor(R.color.brand));
				}
					...
					if (layout != null) {
						...
						if (y + yOffset > top + keyboardSize && y + yOffset < parentView.getMeasuredHeight()) {
							if (!layout.isRtlCharAt(selectionEnd)) {
								...
								canvas.restore();
								endArea.set(xOffset + x, yOffset + y - handleViewSize, xOffset + x + handleViewSize, yOffset + y + handleViewSize);
								...
								canvas.drawPath(path, handleViewPaint);
								canvas.restore();
								endArea.set(xOffset + x - handleViewSize, yOffset + y - handleViewSize, xOffset + x, yOffset + y + handleViewSize);
								endArea.inset(-AndroidUtilities.dp(8), -AndroidUtilities.dp(8));
							}
						}
						else {
					...
					if (layout != null) {
						...
						if (y + yOffset > top + keyboardSize && y + yOffset < parentView.getMeasuredHeight()) {
							if (!layout.isRtlCharAt(selectionStart)) {
								...
								canvas.drawPath(path, handleViewPaint);
								canvas.restore();
								startArea.set(xOffset + x - handleViewSize, yOffset + y - handleViewSize, xOffset + x, yOffset + y + handleViewSize);
								...
								canvas.drawPath(path, handleViewPaint);
								canvas.restore();
								startArea.set(xOffset + x, yOffset + y - handleViewSize, xOffset + x + handleViewSize, yOffset + y + handleViewSize);
								startArea.inset(-AndroidUtilities.dp(8), -AndroidUtilities.dp(8));
							}
						}
						else {
	...
	private ActionMode.Callback createActionCallback() {
		final ActionMode.Callback callback = new ActionMode.Callback() {
			...
			private void updateTranslateButton(Menu menu) {
				...
				menu.getItem(2).setVisible(onTranslateListener != null && ((translateFromLanguage != null && (!translateFromLanguage.equals(translateToLanguage) || translateFromLanguage.equals("und")) && !RestrictedLanguagesSelectActivity.getRestrictedLanguages().contains(translateFromLanguage)) || !LanguageDetector.hasSupport()));
			}
	...
	protected void drawSelection(Canvas canvas, StaticLayout layout, int selectionStart, int selectionEnd, boolean hasStart, boolean hasEnd) {
		...
		float startLeft = layout.getPrimaryHorizontal(selectionStart), endRight = layout.getPrimaryHorizontal(selectionEnd);
		float startBottom = layout.getLineBottom(startLine), endBottom = layout.getLineBottom(endLine);
	...
	private void drawLine(StaticLayout layout, int line, int start, int end, boolean padAtStart, boolean padAtEnd) {
		...
		for (int i = 0; i < tempPath2.rectsCount; ++i) {
			...
			rect.set((int)(rect.left - (padAtStart ? cornerRadius / 2 : 0)), (int)((rect.top - cy) * sy + cy), (int)(rect.right + (padAtEnd ? cornerRadius / 2 : 0)), (int)((rect.bottom - cy) * sy + cy));
			selectionPath.addRect(rect.left, rect.top, rect.right, rect.bottom, Path.Direction.CW);
		}

		if (tempPath2.rectsCount == 0 && !padAtEnd) {
			int left = (int)layout.getPrimaryHorizontal(start), right = (int)layout.getPrimaryHorizontal(end);
	...
	public static class ChatListTextSelectionHelper extends TextSelectionHelper<ChatMessageCell> {
		...
		public void setMessageObject(ChatMessageCell chatMessageCell) {
			...
			if (maybeIsDescription && chatMessageCell.getDescriptionLayout() != null) {
				textArea.set(maybeTextX, maybeTextY, maybeTextX + chatMessageCell.getDescriptionLayout().getWidth(), maybeTextY + chatMessageCell.getDescriptionLayout().getHeight());
			}
			else if (chatMessageCell.hasCaptionLayout()) {
				textArea.set(maybeTextX, maybeTextY, maybeTextX + chatMessageCell.getCaptionLayout().getWidth(), maybeTextY + chatMessageCell.getCaptionLayout().getHeight());
			}
			else if (messageObject != null && messageObject.textLayoutBlocks != null && messageObject.textLayoutBlocks.size() > 0) {
				org.telegram.messenger.messageobject.TextLayoutBlock block = messageObject.textLayoutBlocks.get(messageObject.textLayoutBlocks.size() - 1);
				textArea.set(maybeTextX, maybeTextY, maybeTextX + block.textLayout.getWidth(), (int)(maybeTextY + block.textYOffset + block.textLayout.getHeight()));
			}
			else {
		...
		public void draw(MessageObject messageObject, org.telegram.messenger.messageobject.TextLayoutBlock block, Canvas canvas) {
			...
			if (messageObject.getId() == selectedCellId) {
				...
				if (selectionStart != selectionEnd) {
					if (selectedMessageObject.isOutOwner()) {
						selectionPaint.setColor(selectedView.getContext().getColor(R.color.white_transparent));
						selectionHandlePaint.setColor(selectedView.getContext().getColor(R.color.white_transparent));
					}
					else {
						selectionPaint.setColor(selectedView.getContext().getColor(R.color.brand_transparent));
						selectionHandlePaint.setColor(selectedView.getContext().getColor(R.color.brand_transparent));
					}
		...
		protected int getCharOffsetFromCord(int x, int y, int offsetX, int offsetY, ChatMessageCell cell, boolean maybe) {
			...
			if (isDescription) {
				...
				org.telegram.messenger.messageobject.TextLayoutBlock lastBlock = cell.getMessageObject().textLayoutBlocks.get(cell.getMessageObject().textLayoutBlocks.size() - 1);
			...
			fillLayoutForCoordinates(x, y, cell, layoutBlock, maybe);
		...
		private void fillLayoutForCoordinates(int x, int y, ChatMessageCell cell, LayoutBlock layoutBlock, boolean maybe) {
			if (cell == null) {
				return;
			}
			...
			for (int i = 0; i < messageObject.textLayoutBlocks.size(); i++) {
				org.telegram.messenger.messageobject.TextLayoutBlock block = messageObject.textLayoutBlocks.get(i);
		...
		public void drawCaption(boolean isOut, StaticLayout captionLayout, Canvas canvas) {
			...
			if (isOut) {
				selectionPaint.setColor(context.getColor(R.color.white_transparent));
				selectionHandlePaint.setColor(context.getColor(R.color.white_transparent));
			}
			else {
				selectionPaint.setColor(context.getColor(R.color.brand_transparent));
				selectionHandlePaint.setColor(context.getColor(R.color.brand_transparent));
			}
...
//                  selectedCellEditDate == null && currentEditDate == null)
	...
	public static class ArticleTextSelectionHelper extends TextSelectionHelper<ArticleSelectableView> {
		...
		public void draw(Canvas canvas, ArticleSelectableView view, int i) {
			...
			selectionPaint.setColor(context.getColor(R.color.brand));
			selectionHandlePaint.setColor(context.getColor(R.color.brand));
	...
	private static class ScalablePath extends Path {
		...
		private final ArrayList<RectF> rects = new ArrayList<>(1);
