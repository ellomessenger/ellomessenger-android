...
public class InviteUserCell extends FrameLayout {
	...
	public void update() {
		...
		avatarDrawable.setInfo(currentContact.firstName, currentContact.lastName);

		if (currentName != null) {
			nameTextView.setText(currentName, true);
		}
		else {
			nameTextView.setText(ContactsController.formatName(currentContact.firstName, currentContact.lastName));
		}

		statusTextView.setTextColor(getContext().getColor(R.color.dark_gray));
