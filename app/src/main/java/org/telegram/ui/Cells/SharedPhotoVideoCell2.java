...
public class SharedPhotoVideoCell2 extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (currentMessageObject == null || !imageReceiver.hasBitmapImage() || imageReceiver.getCurrentAlpha() != 1.0f || imageAlpha != 1f) {
			if (SharedPhotoVideoCell2.this.getParent() != null) {
				...
				canvas.drawRect(localPadding, localPadding, localPadding + imageWidth, localPadding + imageHeight, globalGradientView.paint);
			}
			invalidate();
		}
		...
		if (currentMessageObject != null) {
			if (checkBoxProgress > 0) {
				...
				imageReceiver.setImageCoordinates(padding + offset, padding + offset, imageWidth - offset * 2, imageHeight - offset * 2);
			}
			else {
				float localPadding = padding;
				if (crossfadeProgress > 0.5f && crossfadeToColumnsCount != 9 && currentParentColumnsCount != 9) {
				...
				imageReceiver.setImageCoordinates(localPadding, localPadding, imageWidth, imageHeight);
			}
	...
	public void setChecked(final boolean checked, boolean animated) {
		...
		if (checkBoxBase == null) {
			checkBoxBase = new CheckBoxBase(this, 21);
			checkBoxBase.setColor(0, getContext().getColor(R.color.light_background), getContext().getColor(R.color.brand));
