...
public class StickerEmojiCell extends FrameLayout implements NotificationCenter.NotificationCenterDelegate, RecyclerListViewWithOverlayDraw.OverlayView {
	...
	private final boolean fromEmojiPanel;
	private boolean isPremiumSticker;
	private final float premiumAlpha = 1f;
	...
	public void setSticker(TLRPC.Document document, SendMessagesHelper.ImportingSticker path, Object parent, String emoji, boolean showEmoji) {
		...
		if (path != null) {
			stickerPath = path;
			if (path.validated) {
				imageView.setImage(ImageLocation.getForPath(path.path), "80_80", null, null, DocumentObject.getSvgRectThumb(ResourcesCompat.getColor(getContext().getResources(), R.color.light_background, null), 1.0f), 0, path.animated ? "tgs" : null, 0, 1);
			}
			else {
				imageView.setImage(null, null, null, null, DocumentObject.getSvgRectThumb(ResourcesCompat.getColor(getContext().getResources(), R.color.light_background, null), 1.0f), 0, path.animated ? "tgs" : null, 0, 1);
			}
			if (emoji != null) {
	...
	private void updatePremiumStatus(boolean animated) {
		boolean showPremiumLock = isPremiumSticker;
	...
	private void drawInternal(View view, Canvas canvas) {
		...
		imageView.setImageCoordinates(cX - size / 2f, cY - size / 2f, size, size);
		imageView.setAlpha(alpha * premiumAlpha);
		if (scale != 1f) {
			canvas.save();
