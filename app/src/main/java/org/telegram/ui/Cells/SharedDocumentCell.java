...
public class SharedDocumentCell extends FrameLayout implements DownloadController.FileDownloadProgressListener {
	...
	private final RLottieImageView statusImageView;
	...
	public SharedDocumentCell(Context context, int viewType) {
		...
		extTextView.setTextColor(context.getColor(R.color.white));
		extTextView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 14);
		extTextView.setTypeface(Theme.TYPEFACE_BOLD);
		...
		thumbImageView = new BackupImageView(context) {
			...
			protected void onDraw(@NonNull Canvas canvas) {
				float alpha;
				if (thumbImageView.imageReceiver.hasBitmapImage()) {
					alpha = 1.0f - thumbImageView.imageReceiver.getCurrentAlpha();
				}
				else {
					alpha = 1.0f;
				}
		...
		if (viewType == VIEW_TYPE_PICKER) {
			...
			rightDateTextView.setTextColor(context.getColor(R.color.dark_gray));
			...
			captionTextView.setTextColor(context.getColor(R.color.text));
		...
		statusImageView.setColorFilter(new PorterDuffColorFilter(context.getColor(R.color.brand), PorterDuff.Mode.SRC_IN));
		if (viewType == VIEW_TYPE_PICKER) {
		...
		dateTextView.setTextColor(context.getColor(R.color.dark_gray));
	...
	public void setTextAndValueAndTypeAndThumb(@Nullable String text, @Nullable String value, String type, @Nullable String thumb, int resId, boolean divider) {
	...
	public void setDocument(MessageObject messageObject, boolean divider) {
		...
		if (document != null) {
			...
			if (thumb instanceof TLRPC.TL_photoSizeEmpty || thumb == null) {
				...
				thumbImageView.imageReceiver.setNeedsQualityThumb(bigthumb == null);
				thumbImageView.imageReceiver.setShouldGenerateQualityThumb(bigthumb == null);
	...
	private void updateDateView() {
		...
		if (viewType == VIEW_TYPE_GLOBAL_SEARCH) {
			...
			dateTextView.setText(new SpannableStringBuilder().append(fileSize).append(' ').append(dotSpan).append(' ').append(fromName));
