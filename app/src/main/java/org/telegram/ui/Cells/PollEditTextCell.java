...
public class PollEditTextCell extends FrameLayout {
	...
	public PollEditTextCell(Context context, boolean caption, OnClickListener onDelete) {
		...
		if (onDelete != null) {
			...
			deleteImageView.setContentDescription(context.getString(R.string.Delete));
			...
			checkBox.setContentDescription(context.getString(R.string.AccDescrQuizCorrectAnswer));
	...
	public void setShowCheckBox(boolean show, boolean animated) {
		...
		if (animated) {
			...
			checkBoxAnimation.playTogether(ObjectAnimator.ofFloat(checkBox, View.ALPHA, show ? 1.0f : 0.0f), ObjectAnimator.ofFloat(moveImageView, View.ALPHA, show ? 0.0f : 1.0f));
