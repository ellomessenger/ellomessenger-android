...
public class WallpaperUpdater {
	...
	private final WallpaperUpdaterDelegate delegate;
	...
	public void showAlert(final boolean fromTheme) {
		...
		builder.setTitle(parentActivity.getString(R.string.ChoosePhoto), true);

		CharSequence[] items;
		int[] icons;
		if (fromTheme) {
			items = new CharSequence[]{parentActivity.getString(R.string.ChooseTakePhoto), parentActivity.getString(R.string.SelectFromGallery), parentActivity.getString(R.string.SelectColor), parentActivity.getString(R.string.Default)};
			icons = null;
		}
		else {
			items = new CharSequence[]{parentActivity.getString(R.string.ChooseTakePhoto), parentActivity.getString(R.string.SelectFromGallery)};
		...
		if (parentFragment != null) {
			if (parentFragment.getParentActivity() != null) {
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
					if (parentFragment.getParentActivity().checkSelfPermission(Manifest.permission.READ_MEDIA_IMAGES) != PackageManager.PERMISSION_GRANTED || parentFragment.getParentActivity().checkSelfPermission(Manifest.permission.READ_MEDIA_VIDEO) != PackageManager.PERMISSION_GRANTED) {
						parentFragment.getParentActivity().requestPermissions(new String[]{Manifest.permission.READ_MEDIA_IMAGES, Manifest.permission.READ_MEDIA_VIDEO}, BasePermissionsActivity.REQUEST_CODE_EXTERNAL_STORAGE);
						return;
					}
				}
				else {
			...
			PhotoAlbumPickerActivity fragment = new PhotoAlbumPickerActivity(PhotoAlbumPickerActivity.SELECT_TYPE_WALLPAPER, false, false, null, false);
	...
	private void didSelectPhotos(List<SendMessagesHelper.SendingMediaInfo> photos) {
		try {
			if (!photos.isEmpty()) {
