...
public class JoinCallByUrlAlert extends BottomSheet {
	...
	public JoinCallByUrlAlert(final Context context, TLRPC.Chat chat) {
		...
		if (call != null) {
			final var title = TLRPCExtensions.getTitle(call.call);
			...
			final var participantsCount = TLRPCExtensions.getParticipantsCount(call.call);

			if (participantsCount == 0) {
				infoTextView.setText(context.getString(R.string.NoOneJoinedYet));
			}
			else {
				infoTextView.setText(LocaleController.formatPluralString("Participants", participantsCount));
			}
		}
		else {
			percentTextView.setText(chat.title);
		...
		if (ChatObject.isChannelOrGiga(chat)) {
			clearButton.setText(context.getString(R.string.VoipChannelJoinVoiceChatUrl));
		}
		else {
			clearButton.setText(context.getString(R.string.VoipGroupJoinVoiceChatUrl));
		}
