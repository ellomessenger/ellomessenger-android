...
public class PhotoFilterView extends FrameLayout implements FilterShaders.FilterShadersDelegate {
	...
	private final int enhanceTool;
	private final int exposureTool;
	private final int contrastTool;
	private final int saturationTool;
	private final int warmthTool;
	private final int fadeTool;
	private final int highlightsTool;
	private final int shadowsTool;
	private final int vignetteTool;
	private final int grainTool;
	private final int sharpenTool;
	private final int tintShadowsTool;
	private final int tintHighlightsTool;
	private final CurvesToolValue curvesToolValue;
	private final FrameLayout toolsView;
	private final TextView doneTextView;
	private final TextView cancelTextView;
	private final TextureView textureView;
	private final RecyclerListView recyclerListView;
	private final FrameLayout blurLayout;
	private final PhotoFilterBlurControl blurControl;
	private final PhotoFilterCurvesControl curvesControl;
	private final TextView blurOffButton;
	private final TextView blurRadialButton;
	private final TextView blurLinearButton;
	private final FrameLayout curveLayout;
	private final RadioButton[] curveRadioButton = new RadioButton[4];
	private final PaintingOverlay paintingOverlay;
	private final boolean isMirrored;
	private final boolean inBubbleMode;
	private final ImageView tuneItem;
	...
	public PhotoFilterView(Context context, VideoEditTextureView videoTextureView, Bitmap bitmap, int rotation, MediaController.SavedFilterState state, PaintingOverlay overlay, int hasFaces, boolean mirror) {
		...
		cancelTextView.setTypeface(Theme.TYPEFACE_BOLD);
		...
		doneTextView.setTypeface(Theme.TYPEFACE_BOLD);
	...
	public boolean hasChanges() {
		if (lastState != null) {
			...
			return enhanceValue != 0 || contrastValue != 0 || highlightsValue != 0 || exposureValue != 0 || warmthValue != 0 || saturationValue != 0 || vignetteValue != 0 || shadowsValue != 0 || grainValue != 0 || sharpenValue != 0 || fadeValue != 0 || softenSkinValue != 0 || tintHighlightsColor != 0 || tintShadowsColor != 0 || !curvesToolValue.shouldBeSkipped();
		}
	}
	...
	public static class CurvesValue {
		...
		public float[] interpolateCurve() {
			float[] points = new float[]{-0.001f, blacksLevel / 100.0f, 0.0f, blacksLevel / 100.0f, 0.25f, shadowsLevel / 100.0f, 0.5f, midtonesLevel / 100.0f, 0.75f, highlightsLevel / 100.0f, 1f, whitesLevel / 100.0f, 1.001f, whitesLevel / 100.0f};
