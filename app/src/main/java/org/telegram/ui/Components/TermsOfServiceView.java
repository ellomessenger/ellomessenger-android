...
public class TermsOfServiceView extends FrameLayout {
	...
	private TLRPC.TLHelpTermsOfService currentTos;
	...
	public TermsOfServiceView(final Context context) {
		...
		declineTextView.setTypeface(Theme.TYPEFACE_BOLD);
		...
		declineTextView.setOnClickListener(view -> {
			...
			builder.setPositiveButton(LocaleController.getString("DeclineDeactivate", R.string.DeclineDeactivate), (dialog, which) -> {
				...
				builder12.setPositiveButton(LocaleController.getString("Deactivate", R.string.Deactivate), (dialogInterface, i) -> {
					...
					var req = new TLRPC.TLAccountDeleteAccount();
		...
		acceptTextView.setOnClickListener(view -> {
			if (currentTos.minAgeConfirm != 0) {
				...
				builder.setMessage(LocaleController.formatString("TosAgeText", R.string.TosAgeText, LocaleController.formatPluralString("Years", currentTos.minAgeConfirm)));
				builder.show();
			}
			else {
				accept();
	...
	private void accept() {
		...
		var req = new TLRPC.TLHelpAcceptTermsOfService();
		req.id = currentTos.id;
		ConnectionsManager.getInstance(currentAccount).sendRequest(req);
	}

	public void show(int account, TLRPC.TLHelpTermsOfService tos) {
