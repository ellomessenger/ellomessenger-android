...
@SuppressLint({"NotifyDataSetChanged", "AppCompatCustomView"})
public class EmojiView extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
	...
	private final ArrayList<Tab> currentTabs = new ArrayList<>();
	...
	private final EmojiTabsStrip emojiTabs;
	private final View emojiTabsShadow;
	private final EmojiGridView emojiGridView;
	...
	private final RecyclerAnimationScrollHelper emojiScrollHelper;
	...
	private final Map<String, TLRPC.TLMessagesBotResults> gifCache = new HashMap<>();
	...
	private final ArrayList<TLRPC.TLMessagesStickerSet> stickerSets = new ArrayList<>();
	private int groupStickerPackNum;
	private int groupStickerPackPosition;
	private boolean groupStickersHidden;
	private TLRPC.TLMessagesStickerSet groupStickerSet;
	...
	private final ArrayList<TLRPC.StickerSetCovered> featuredEmojiSets = new ArrayList<>();
	private final ArrayList<Long> expandedEmojiSets = new ArrayList<>();
	public ArrayList<Long> installedEmojiSets = new ArrayList<>();
	private final ArrayList<EmojiPack> emojipacksProcessed = new ArrayList<>();
	private final HashMap<Long, Utilities.Callback<TLRPC.TLMessagesStickerSet>> toInstall = new HashMap<>();
	...
	private final Object outlineProvider;
	...
	public interface EmojiViewDelegate {
		...
		default void onStickerSelected(View view, TLRPC.Document sticker, String query, Object parent, SendAnimationData sendAnimationData, boolean notify, int scheduleDate) {
		...
		default void onEmojiSettingsClick(List<TLRPC.TLMessagesStickerSet> frozenEmojiPacks) {
		...
		default void onShowStickerSet(TLRPC.TLStickerSet stickerSet, TLRPC.InputStickerSet inputStickerSet) {
		...
		public SearchField(Context context, int type) {
			...
			searchEditText.addTextChangedListener(new TextWatcher() {
				...
				public void afterTextChanged(Editable s) {
					...
					if (show != showed) {
						clearSearchImageView.animate().alpha(show ? 1.0f : 0.0f).setDuration(150).scaleX(show ? 1.0f : 0.1f).scaleY(show ? 1.0f : 0.1f).start();
					}
					if (type == 0) {
...
//                }
				if (delegate != null) {
					...
					if (!MessageObject.isFreeEmoji(document) && !UserConfig.getInstance(currentAccount).isPremium() && !(delegate != null && delegate.isUserSelf())) {
						...
						BulletinFactory factory = fragment != null ? BulletinFactory.of(fragment) : BulletinFactory.of(bulletinContainer);
						if (premiumBulletin || fragment == null) {
							factory.createEmojiBulletin(document, AndroidUtilities.replaceTags(LocaleController.getString("UnlockPremiumEmojiHint", R.string.UnlockPremiumEmojiHint)), LocaleController.getString("PremiumMore", R.string.PremiumMore), EmojiView.this::openPremiumAnimatedEmojiFeature).show();
						}
						else {
							factory.createSimpleBulletin(R.raw.saved_messages, AndroidUtilities.replaceTags(LocaleController.getString("UnlockPremiumEmojiHint2", R.string.UnlockPremiumEmojiHint2)), LocaleController.getString("Open", R.string.Open), () -> {
	...
	private class EmojiColorPickerView extends View {
		...
		private final AnimatedFloat selectionAnimated = new AnimatedFloat(this, 125, CubicBezierInterpolator.EASE_OUT_QUINT);
	...
	public EmojiView(BaseFragment fragment, boolean needAnimatedEmoji, boolean needStickers, boolean needGif, final Context context, boolean needSearch, final TLRPC.ChatFull chatFull, ViewGroup parentView) {
		super(context);
		...
		tabIcons = new Drawable[]{Theme.createEmojiIconSelectorDrawable(context, R.drawable.smiles_tab_smiles, getThemedColor(Theme.key_chat_emojiPanelBackspace), getThemedColor(Theme.key_chat_emojiPanelIconSelected)), Theme.createEmojiIconSelectorDrawable(context, R.drawable.smiles_tab_gif, getThemedColor(Theme.key_chat_emojiPanelBackspace), getThemedColor(Theme.key_chat_emojiPanelIconSelected)), Theme.createEmojiIconSelectorDrawable(context, R.drawable.smiles_tab_stickers, getThemedColor(Theme.key_chat_emojiPanelBackspace), getThemedColor(Theme.key_chat_emojiPanelIconSelected))};

		stickerIcons = new Drawable[]{Theme.createEmojiIconSelectorDrawable(context, R.drawable.msg_emoji_recent, getThemedColor(Theme.key_chat_emojiPanelIcon), getThemedColor(Theme.key_chat_emojiPanelIconSelected)), Theme.createEmojiIconSelectorDrawable(context, R.drawable.emoji_tabs_faves, getThemedColor(Theme.key_chat_emojiPanelIcon), getThemedColor(Theme.key_chat_emojiPanelIconSelected)), Theme.createEmojiIconSelectorDrawable(context, R.drawable.emoji_tabs_new3, getThemedColor(Theme.key_chat_emojiPanelIcon), getThemedColor(Theme.key_chat_emojiPanelIconSelected)), new LayerDrawable(new Drawable[]{searchIconDrawable = Theme.createEmojiIconSelectorDrawable(context, R.drawable.emoji_tabs_new1, getThemedColor(Theme.key_chat_emojiPanelIcon), getThemedColor(Theme.key_chat_emojiPanelIconSelected)), searchIconDotDrawable = Theme.createEmojiIconSelectorDrawable(context, R.drawable.emoji_tabs_new2, getThemedColor(Theme.key_chat_emojiPanelStickerPackSelectorLine), getThemedColor(Theme.key_chat_emojiPanelStickerPackSelectorLine))})};

		gifIcons = new Drawable[]{Theme.createEmojiIconSelectorDrawable(context, R.drawable.msg_emoji_recent, getThemedColor(Theme.key_chat_emojiPanelIcon), getThemedColor(Theme.key_chat_emojiPanelIconSelected)), Theme.createEmojiIconSelectorDrawable(context, R.drawable.stickers_gifs_trending, getThemedColor(Theme.key_chat_emojiPanelIcon), getThemedColor(Theme.key_chat_emojiPanelIconSelected)),};

		emojiTitles = new String[]{LocaleController.getString("Emoji1", R.string.Emoji1), LocaleController.getString("Emoji2", R.string.Emoji2), LocaleController.getString("Emoji3", R.string.Emoji3), LocaleController.getString("Emoji4", R.string.Emoji4), LocaleController.getString("Emoji5", R.string.Emoji5), LocaleController.getString("Emoji6", R.string.Emoji6), LocaleController.getString("Emoji7", R.string.Emoji7), LocaleController.getString("Emoji8", R.string.Emoji8)};
		...
		emojiGridView.setOnItemLongClickListener(new RecyclerListView.OnItemLongClickListener() {
			@Override
			public boolean onItemClick(@NonNull View view, int position) {
				if (view instanceof ImageViewEmoji viewEmoji) {
		...
		emojiLayoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
			...
			public int getSpanSize(int position) {
				if (emojiGridView.getAdapter() == emojiSearchAdapter) {
					...
					if (needEmojiSearch && position == 0 || position == emojiAdapter.trendingRow || emojiAdapter.positionToSection.indexOfKey(position) >= 0 || emojiAdapter.positionToUnlock.indexOfKey(position) >= 0) {
						return emojiLayoutManager.getSpanCount();
					}
				}
				return 1;
		...
		emojiGridView.addOnScrollListener(new TypedScrollListener(Type.EMOJIS) {
			@Override
			public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
		...
		emojiTabs = new EmojiTabsStrip(context, true, needAnimatedEmoji, fragment != null ? () -> {
			if (delegate != null) {
				delegate.onEmojiSettingsClick(emojiAdapter.frozenEmojiPacks);
			}
		} : null) {
			@Override
			protected boolean isInstalled(@NonNull EmojiPack pack) {
...
//                        }
		...
		if (needSearch) {
			...
			emojiSearchField.searchEditText.setOnFocusChangeListener((v, hasFocus) -> {
				if (hasFocus) {
		...
		if (needStickers) {
			...
			stickersTab = new DraggableScrollSlidingTabStrip(context) {
				...
				private void swapListElements(List<TLRPC.TLMessagesStickerSet> list, int index1, int index2) {
					final TLRPC.TLMessagesStickerSet set1 = list.remove(index1);
					list.add(index2, set1);
				}

				private void sendReorder() {
					MediaDataController.getInstance(currentAccount).calcNewHash(MediaDataController.TYPE_IMAGE);
					TLRPC.TLMessagesReorderStickerSets req = new TLRPC.TLMessagesReorderStickerSets();
					req.masks = false;
					req.emojis = false;
		...
		pickerViewPopup.getContentView().setOnKeyListener((v, keyCode, event) -> {
			if (keyCode == KeyEvent.KEYCODE_MENU && event.getRepeatCount() == 0 && event.getAction() == KeyEvent.ACTION_UP && pickerViewPopup.isShowing()) {
			...
			if (emojiTouchedView != null) {
				if (event.getAction() == MotionEvent.ACTION_UP || event.getAction() == MotionEvent.ACTION_CANCEL) {
					if (pickerViewPopup != null && pickerViewPopup.isShowing()) {
						...
						String color = switch (pickerView.getSelection()) {
							case 1 -> "\uD83C\uDFFB";
							case 2 -> "\uD83C\uDFFC";
							case 3 -> "\uD83C\uDFFD";
							case 4 -> "\uD83C\uDFFE";
...
//            drawDashedOutlines(canvas);
		...
		class DrawingInBackgroundLine extends DrawingInBackgroundThreadDrawable {
			...
			private final OvershootInterpolator appearScaleInterpolator = new OvershootInterpolator(3f);
		...
		public boolean dispatchTouchEvent(MotionEvent ev) {
			...
			if (down || up || cancel) {
				...
				if (down) {
					...
					if (touchChild != null && touch != null && Math.sqrt(Math.pow(x - touch.x, 2) + Math.pow(y - touch.y, 2)) < AndroidUtilities.touchSlop * 3 &&
//                        (SystemClock.elapsedRealtime() - touch.time) <= ViewConfiguration.getTapTimeout() * 1.2f &&
							!cancel && (!pickerViewPopup.isShowing() || SystemClock.elapsedRealtime() - touch.time < ViewConfiguration.getLongPressTimeout())) {
						...
						else if (view instanceof EmojiPackExpand button) {
		...
		private final SparseIntArray headerWidthsCache = new SparseIntArray();
		private final AnimatedFloat premiumT = new AnimatedFloat(this, 350, CubicBezierInterpolator.EASE_OUT_QUINT);
		...
		public void drawDashedOutlines(Canvas canvas) {
			...
			for (int i = 0; i < emojiAdapter.packStartPosition.size(); ++i) {
				...
				if (positionInGroup == 0) {
					...
					top = child.getTop() + child.getTranslationY() - AndroidUtilities.dp(32 - 25) - (positionInGroup - 1) / Math.max(1, emojiLayoutManager.getSpanCount()) * itemSize - AndroidUtilities.dp(32 - 25);
				}
				float bottom;
	...
	private class EmojiPackButton extends FrameLayout {
		...
		public void updateInstall(boolean installed, boolean animated) {
			CharSequence text = installed ? LocaleController.getString("Added", R.string.Added) : LocaleController.formatString("AddStickersCount", R.string.AddStickersCount, lastTitle);
			...
			if (animated) {
				...
				installFadeAway.addUpdateListener(anm -> addButtonView.setAlpha((float)anm.getAnimatedValue()));
	...
	private class EmojiPackHeader extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
		...
		public EmojiPackHeader(Context context) {
			...
			headerView.setTypeface(Theme.TYPEFACE_BOLD);
			...
			addButtonView.setTypeface(Theme.TYPEFACE_BOLD);
			...
			addButtonView.setOnClickListener(e -> {
				...
				for (int i = 0; i < emojiGridView.getChildCount(); ++i) {
					if (emojiGridView.getChildAt(i) instanceof EmojiPackExpand) {
						...
						if (j >= 0) {
							...
							if (section >= 0 && section < emojipacksProcessed.size() && emojipacksProcessed.get(section) != null && pack != null && emojipacksProcessed.get(section).set.id == pack.set.id) {
								position = j;
								expandButton = child;
								break;
							}
				...
				TLRPC.TLMessagesStickerSet stickerSet = MediaDataController.getInstance(currentAccount).getStickerSet(inputStickerSetID, true);
			...
			removeButtonView.setTypeface(Theme.TYPEFACE_BOLD);
		...
		protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
			...
			super.onMeasure(MeasureSpec.makeMeasureSpec(MeasureSpec.getSize(widthMeasureSpec), MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(currentButtonState == BUTTON_STATE_EMPTY ? 32 : 42), MeasureSpec.EXACTLY));
		}
		...
		public void didReceivedNotification(int id, int account, Object... args) {
			if (id == NotificationCenter.groupStickersDidLoad) {
				if (toInstall instanceof TLRPC.TLInputStickerSetID toInstallId) {
					TLRPC.TLMessagesStickerSet stickerSet = MediaDataController.getInstance(currentAccount).getStickerSetById(toInstallId.id);
				...
				if (toUninstall instanceof TLRPC.TLInputStickerSetID toUninstallId) {
					TLRPC.TLMessagesStickerSet stickerSet = MediaDataController.getInstance(currentAccount).getStickerSetById(toUninstallId.id);
					if (stickerSet != null && stickerSet.set != null) {
						uninstall(stickerSet);
						toUninstall = null;
					}
		...
		private void install(TLRPC.TLMessagesStickerSet set) {
		...
		private void uninstall(TLRPC.TLMessagesStickerSet set) {
		...
		protected void onDraw(@NonNull Canvas canvas) {
			if (divider) {
				if (dividerPaint == null) {
					...
					dividerPaint.setColor(getContext().getColor(R.color.divider));
				}
		...
		public void updateState(int state, boolean animated) {
			...
			if (animated) {
				...
				stateAnimator.playTogether(ObjectAnimator.ofFloat(lockView, TRANSLATION_X, state == BUTTON_STATE_LOCKED ? 0 : -AndroidUtilities.dp(16)), ObjectAnimator.ofFloat(lockView, ALPHA, state == BUTTON_STATE_LOCKED ? 1f : 0), ObjectAnimator.ofFloat(headerView, TRANSLATION_X, state == BUTTON_STATE_LOCKED ? AndroidUtilities.dp(16) : 0), ObjectAnimator.ofFloat(premiumButtonView, ALPHA, state == BUTTON_STATE_LOCKED ? 1 : 0), ObjectAnimator.ofFloat(premiumButtonView, SCALE_X, state == BUTTON_STATE_LOCKED ? 1 : .6f), ObjectAnimator.ofFloat(premiumButtonView, SCALE_Y, state == BUTTON_STATE_LOCKED ? 1 : .6f), ObjectAnimator.ofFloat(addButtonView, ALPHA, state == BUTTON_STATE_ADD ? 1 : 0), ObjectAnimator.ofFloat(addButtonView, SCALE_X, state == BUTTON_STATE_ADD ? 1 : .6f), ObjectAnimator.ofFloat(addButtonView, SCALE_Y, state == BUTTON_STATE_ADD ? 1 : .6f), ObjectAnimator.ofFloat(removeButtonView, ALPHA, state == BUTTON_STATE_REMOVE ? 1 : 0), ObjectAnimator.ofFloat(removeButtonView, SCALE_X, state == BUTTON_STATE_REMOVE ? 1 : .6f), ObjectAnimator.ofFloat(removeButtonView, SCALE_Y, state == BUTTON_STATE_REMOVE ? 1 : .6f));
	...
	public void openEmojiPackAlert(TLRPC.TLStickerSet set) {
		...
		inputStickerSetID.accessHash = set.accessHash;
...
//                    }
	...
	private void startStopVisibleGifs(boolean start) {
		...
		for (int a = 0; a < count; a++) {
			...
			if (child instanceof ContextLinkCell cell) {
	...
	public List<String> getRecentEmoji() {
		...
		for (int a = 0; a < 3; a++) {
			...
			if (searchField == currentField && delegate != null && delegate.isExpanded()) {
				...
				if (tabStrip != null && a != 2) {
					searchAnimation.playTogether(ObjectAnimator.ofFloat(tabStrip, View.TRANSLATION_Y, -AndroidUtilities.dp(40)), ObjectAnimator.ofFloat(gridView, View.TRANSLATION_Y, -AndroidUtilities.dp(36)), ObjectAnimator.ofFloat(currentField, View.TRANSLATION_Y, AndroidUtilities.dp(0)));
				}
				else {
					searchAnimation.playTogether(ObjectAnimator.ofFloat(gridView, View.TRANSLATION_Y, -AndroidUtilities.dp(36)), ObjectAnimator.ofFloat(currentField, View.TRANSLATION_Y, AndroidUtilities.dp(0)));
				}
	...
	public void closeSearch(boolean animated, long scrollToSet) {
		...
		if (currentItem == 2 && scrollToSet != -1) {
			TLRPC.TLMessagesStickerSet set = MediaDataController.getInstance(currentAccount).getStickerSetById(scrollToSet);
			if (set != null) {
		...
		for (int a = 0; a < 3; a++) {
			...
			if (a == currentItem && animated) {
				searchAnimation = new AnimatorSet();
				if (tabStrip != null && a != 2) {
					searchAnimation.playTogether(ObjectAnimator.ofFloat(tabStrip, View.TRANSLATION_Y, 0), ObjectAnimator.ofFloat(gridView, View.TRANSLATION_Y, AndroidUtilities.dp(36) - searchFieldHeight), ObjectAnimator.ofFloat(currentField, View.TRANSLATION_Y, AndroidUtilities.dp(36) - searchFieldHeight));
				}
				else {
					searchAnimation.playTogether(ObjectAnimator.ofFloat(gridView, View.TRANSLATION_Y, AndroidUtilities.dp(36) - searchFieldHeight), ObjectAnimator.ofFloat(currentField, View.TRANSLATION_Y, -searchFieldHeight));
				}
	...
	private void showStickerSettingsButton(boolean show, boolean animated) {
		...
		if (animated) {
			...
			stickersButtonAnimation.playTogether(ObjectAnimator.ofFloat(stickerSettingsButton, View.ALPHA, show ? 1.0f : 0.0f), ObjectAnimator.ofFloat(stickerSettingsButton, View.SCALE_X, show ? 1.0f : 0.0f), ObjectAnimator.ofFloat(stickerSettingsButton, View.SCALE_Y, show ? 1.0f : 0.0f));
	...
	private void showBottomTab(boolean show, boolean animated) {
		...
		if (animated) {
			...
			bottomTabContainerAnimation.playTogether(ObjectAnimator.ofFloat(bottomTabContainer, View.TRANSLATION_Y, show ? 0 : AndroidUtilities.dp(needEmojiSearch ? 45 : 50)), ObjectAnimator.ofFloat(bulletinContainer, View.TRANSLATION_Y, needEmojiSearch ? (show ? 0 : AndroidUtilities.dp(45)) : bulletinContainer.getTranslationY()), ObjectAnimator.ofFloat(shadowLine, View.TRANSLATION_Y, show ? 0 : AndroidUtilities.dp(45)));
			...
			if (needEmojiSearch) {
				bulletinContainer.setTranslationY(show ? 0 : AndroidUtilities.dp(45));
			}
	...
	private View getTabsForType(@Type int type) {
		return switch (type) {
			case Type.STICKERS -> stickersTab;
			case Type.EMOJIS -> emojiTabs;
			case Type.GIFS -> gifTabs;
			default -> throw new IllegalArgumentException("Unexpected argument: " + type);
		};
	}

	private RecyclerListView getListViewForType(@Type int type) {
		return switch (type) {
			case Type.STICKERS -> stickersGridView;
			case Type.EMOJIS -> emojiGridView;
			case Type.GIFS -> gifGridView;
			default -> throw new IllegalArgumentException("Unexpected argument: " + type);
		};
	}
	...
	private GridLayoutManager getLayoutManagerForType(@Type int type) {
		return switch (type) {
			case Type.STICKERS -> stickersLayoutManager;
			case Type.EMOJIS -> emojiLayoutManager;
			case Type.GIFS -> gifLayoutManager;
			default -> throw new IllegalArgumentException("Unexpected argument: " + type);
		};
	}
	...
	private SearchField getSearchFieldForType(@Type int type) {
		return switch (type) {
			case Type.STICKERS -> stickersSearchField;
			case Type.EMOJIS -> emojiSearchField;
			case Type.GIFS -> gifSearchField;
			default -> throw new IllegalArgumentException("Unexpected argument: " + type);
		};
	}
	...
	private void checkGifSearchFieldScroll(boolean isLayout) {
		if (gifGridView != null && gifGridView.getAdapter() instanceof GifAdapter adapter) {
	...
	ArrayList<TLRPC.TLMessagesStickerSet> frozenStickerSets;
	...
	private void updateStickerTabs(boolean updateStickerSets) {
		...
		List<TLRPC.StickerSetCovered> featured = mediaDataController.getFeaturedStickerSets();
		...
		List<TLRPC.TLMessagesStickerSet> packs = frozenStickerSets;
		for (int i = 0; i < primaryInstallingStickerSets.length; i++) {
			...
			if (installingStickerSet != null) {
				final TLRPC.TLMessagesStickerSet pack = mediaDataController.getStickerSetById(installingStickerSet.set.id);
				if (pack != null && !pack.set.archived) {
					primaryInstallingStickerSets[i] = null;
				}
				else {
					final TLRPC.TLMessagesStickerSet set = new TLRPC.TLMessagesStickerSet();
					set.set = installingStickerSet.set;

					final var covers = TLRPCExtensions.getCovers(installingStickerSet);
					final var cover = TLRPCExtensions.getCover(installingStickerSet);

					if (cover != null) {
						set.documents.add(cover);
					}
					else if (covers != null && !covers.isEmpty()) {
						set.documents.addAll(covers);
					}
		...
		if (info != null) {
			...
			if (info.stickerset != null) {
				TLRPC.TLMessagesStickerSet pack = mediaDataController.getGroupStickerSetById(info.stickerset);
				if (pack != null && pack.documents != null && !pack.documents.isEmpty() && pack.set != null) {
					...
					set.documents.addAll(pack.documents);
					set.packs.addAll(pack.packs);
					set.set = pack.set;
					...
					groupStickerSet = info.canSetStickers ? set : null;
				}
			}
			else if (info.canSetStickers) {
				TLRPC.TLMessagesStickerSet pack = new TLRPC.TLMessagesStickerSet();
				if (groupStickersHidden) {
					groupStickerPackNum = stickerSets.size();
					stickerSets.add(pack);
				}
		...
		for (int a = 0; a < stickerSets.size(); a++) {
			if (a == groupStickerPackNum) {
				...
				TLRPC.TLMessagesStickerSet stickerSet = stickerSets.get(a);
	...
	public void addRecentSticker(TLRPC.Document document) {
		...
		recentStickers = new ArrayList<>(MediaDataController.getInstance(currentAccount).getRecentStickers(MediaDataController.TYPE_IMAGE));
	...
	public void updateColors() {
		...
		for (Drawable tabIcon : tabIcons) {
			Theme.setEmojiDrawableColor(tabIcon, getThemedColor(Theme.key_chat_emojiBottomPanelIcon), false);
			Theme.setEmojiDrawableColor(tabIcon, getThemedColor(Theme.key_chat_emojiPanelIconSelected), true);
		}
		if (emojiTabs != null) {
			emojiTabs.updateColors();
		}
		for (Drawable stickerIcon : stickerIcons) {
			Theme.setEmojiDrawableColor(stickerIcon, getThemedColor(Theme.key_chat_emojiPanelIcon), false);
			Theme.setEmojiDrawableColor(stickerIcon, getThemedColor(Theme.key_chat_emojiPanelIconSelected), true);
		}
		for (Drawable gifIcon : gifIcons) {
			Theme.setEmojiDrawableColor(gifIcon, getThemedColor(Theme.key_chat_emojiPanelIcon), false);
			Theme.setEmojiDrawableColor(gifIcon, getThemedColor(Theme.key_chat_emojiPanelIconSelected), true);
		}
	...
	private void checkDocuments(boolean isGif) {
		if (isGif) {
			...
			recentStickers = new ArrayList<>(MediaDataController.getInstance(currentAccount).getRecentStickers(MediaDataController.TYPE_IMAGE));
			favouriteStickers = new ArrayList<>(MediaDataController.getInstance(currentAccount).getRecentStickers(MediaDataController.TYPE_FAVE));
			if (UserConfig.getInstance(currentAccount).isPremium()) {
				premiumStickers = new ArrayList<>(MediaDataController.getInstance(currentAccount).getRecentStickers(MediaDataController.TYPE_PREMIUM_STICKERS));
			}
			else {
			...
			for (int a = 0; a < favouriteStickers.size(); a++) {
				...
				for (int b = 0; b < recentStickers.size(); b++) {
					...
					if (recSticker instanceof TLRPC.TLDocument recDocument && favSticker instanceof TLRPC.TLDocument favDocument) {
						if (recDocument.dcId == favDocument.dcId && recSticker.id == favSticker.id) {
	...
	private void updateRecentGifs() {
		...
		recentGifs = new ArrayList<>(MediaDataController.getInstance(currentAccount).getRecentGifs());
	...
	public void showStickerBanHint(boolean gif) {
		...
		if (!ChatObject.hasAdminRights(chat) && chat.defaultBannedRights != null && chat.defaultBannedRights.sendStickers) {
			if (gif) {
			...
			if (AndroidUtilities.isBannedForever(chat.bannedRights)) {
				if (gif) {
				...
				if (gif) {
					mediaBanTooltip.setText(LocaleController.formatString("AttachGifRestricted", R.string.AttachGifRestricted, LocaleController.formatDateForBan((long)chat.bannedRights.untilDate)));
				}
				else {
					mediaBanTooltip.setText(LocaleController.formatString("AttachStickersRestricted", R.string.AttachStickersRestricted, LocaleController.formatDateForBan((long)chat.bannedRights.untilDate)));
				}
			}
		}
	...
	private void updateVisibleTrendingSets() {
		...
		try {
			...
			for (int a = 0; a < count; a++) {
				...
				if (child instanceof FeaturedStickerSetInfoCell cell) {
					RecyclerListView.Holder holder = (RecyclerListView.Holder)stickersGridView.getChildViewHolder(child);
					if (holder == null) {
						continue;
					}
					List<Long> unreadStickers = MediaDataController.getInstance(currentAccount).getUnreadStickerSets();
					TLRPC.StickerSetCovered stickerSetCovered = cell.getStickerSet();
					boolean unread = unreadStickers.contains(stickerSetCovered.set.id);
	...
	public void didReceivedNotification(int id, int account, Object... args) {
		if (id == NotificationCenter.stickersDidLoad) {
			...
			if (toInstall.containsKey((Long)args[0]) && args.length >= 2) {
				...
				TLRPC.TLMessagesStickerSet stickerSet = (TLRPC.TLMessagesStickerSet)args[1];
				Utilities.Callback<TLRPC.TLMessagesStickerSet> onInstalled = toInstall.get(packId);
				if (onInstalled != null && stickerSet != null) {
					var callback = toInstall.remove(packId);
					if (callback != null) {
	...
	private class TrendingAdapter extends RecyclerListView.SelectionAdapter {
		...
		public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
			...
			TLRPC.Document document = TLRPCExtensions.getCover(set);
			var covers = TLRPCExtensions.getCovers(set);

			if (covers != null && !covers.isEmpty()) {
				document = covers.get(0);
			}
			TLObject object = FileLoader.getClosestPhotoSizeWithSize(set.set.thumbs, 90);
			SvgHelper.SvgDrawable svgThumb = DocumentObject.getSvgThumb(set.set.thumbs, ResourcesCompat.getColor(getContext().getResources(), R.color.light_background, null), 0.2f);
			if (svgThumb != null) {
			...
			if (object instanceof TLRPC.Document) {
				...
				int thumbVersion = set.set.thumbVersion;
	...
	private class StickersGridAdapter extends RecyclerListView.SelectionAdapter {
		...
		public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
			...
			switch (viewType) {
				...
				case 1:
					...
					break;
				case 2:
					StickerSetNameCell nameCell = new StickerSetNameCell(context, false);
					view = nameCell;
						...
						if (holder != null) {
							if (holder.getAdapterPosition() == groupStickerPackPosition) {
								...
								if (object == recentStickers) {
									AlertDialog alertDialog = new AlertDialog.Builder(context).setTitle(LocaleController.getString(R.string.ClearRecentStickersAlertTitle)).setMessage(LocaleController.getString(R.string.ClearRecentStickersAlertMessage)).setPositiveButton(LocaleController.getString(R.string.ClearButton), (dialog, which) -> MediaDataController.getInstance(currentAccount).clearRecentStickers()).setNegativeButton(LocaleController.getString(R.string.Cancel), null).create();
									alertDialog.show();
				...
				case 6:
					...
					listView.setOnItemClickListener((view1, position) -> openTrendingStickers((TLRPC.StickerSetCovered)view1.getTag()));
					view = listView;
		...
		private void updateItems() {
			...
			cache.clear();
			totalItems = 0;
			List<TLRPC.TLMessagesStickerSet> packs = stickerSets;
			int startRow = 0;
	...
	public static class EmojiPackExpand extends FrameLayout {
		...
		public EmojiPackExpand(Context context) {
			super(context);
			textView = new TextView(context);
			textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 13);
			textView.setTextColor(context.getColor(R.color.background));
			textView.setBackground(Theme.createRoundRectDrawable(AndroidUtilities.dp(11), ColorUtils.setAlphaComponent(Theme.getColor(Theme.key_chat_emojiPanelStickerSetName), 99)));
	...
	public static class CustomEmoji {
		public TLRPC.TLMessagesStickerSet stickerSet;
	...
	public static class EmojiPack {
		public int index;
		public TLRPC.TLStickerSet set;
		public List<TLRPC.Document> documents;
		public boolean free;
	...
	private class EmojiGridAdapter extends RecyclerListView.SelectionAdapter {
		...
		private final int trendingHeaderRow = -1;
		private final int trendingRow = -1;
		private int firstTrendingRow = -1;

		private ArrayList<TLRPC.TLMessagesStickerSet> frozenEmojiPacks;
		private final ArrayList<Integer> rowHashCodes = new ArrayList<>();
		...
		private final SparseIntArray positionToUnlock = new SparseIntArray();
		private final SparseIntArray positionToExpand = new SparseIntArray();
		private final ArrayList<Integer> packStartPosition = new ArrayList<>();
		...
		public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
			...
			switch (viewType) {
					...
					listView.setOnItemClickListener((item, position) -> {
						...
						for (int i = 0; i < featuredStickerSets.size(); ++i) {
							...
							if (set != null && set.set != null) {
								...
								inputStickerSet.accessHash = set.set.accessHash;
								inputStickerSets.add(inputStickerSet);
							}
						}
						MediaDataController.getInstance(currentAccount).markFeaturedStickersAsRead(true, true);
						if (fragment != null) {
							fragment.showDialog(new EmojiPacksAlert(fragment, getContext(), inputStickerSets));
						}
						else {
							new EmojiPacksAlert(null, getContext(), inputStickerSets).show();
						}
					});
					view = listView;
					break;
				...
				case VIEW_TYPE_EXPAND:
					view = new EmojiPackExpand(getContext());
					break;
...
//                            } else {
//                                TLRPC.TLMessagesStickerSet stickerSet = MediaDataController.getInstance(currentAccount).getStickerSetById(expandPack.set.id);
//                                if (stickerSet == null || stickerSet.set == null) {
...
//                                    inputStickerSetID.accessHash = expandPack.set.accessHash;
...
//                    break;
		...
		public void processEmoji(boolean updatEmojipacks) {
			...
			ArrayList<TLRPC.TLMessagesStickerSet> installedEmojipacks = frozenEmojiPacks;
			boolean isPremium = UserConfig.getInstance(currentAccount).isPremium();
			int index = 0;
			if (!isPremium) {
				for (int i = 0; i < installedEmojipacks.size(); ++i) {
					TLRPC.TLMessagesStickerSet set = installedEmojipacks.get(i);
...
//                }
				...
				pack.installed = mediaDataController.isStickerPackInstalled(set.set.id);
				pack.set = set.set;
				pack.documents = set instanceof TLRPC.TLStickerSetFullCovered ? ((TLRPC.TLStickerSetFullCovered)set).documents : TLRPCExtensions.getCovers(set);
				pack.index = index++;
...
//                trendingRow = -1;
//            }
			List<String> recent = getRecentEmoji();
			if (emojiTabs != null) {
...
//                    }
		...
		public void notifyDataSetChanged(boolean updateEmojipack) {
			...
			final MediaDataController mediaDataController = MediaDataController.getInstance(currentAccount);
			List<TLRPC.StickerSetCovered> featured = mediaDataController.getFeaturedEmojiSets();
...
//            super.notifyDataSetChanged();
	...
	private class EmojiSearchAdapter extends RecyclerListView.SelectionAdapter {
		private List<MediaDataController.KeywordResult> result = new ArrayList<>();
			...
			switch (viewType) {
				...
				default:
					...
					imageView.setOnClickListener(v -> {
						...
						textView1.setText(LocaleController.getString("EmojiSuggestions", R.string.EmojiSuggestions));
						textView1.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 15);
						textView1.setTextColor(getThemedColor(Theme.key_dialogTextBlue2));
						textView1.setGravity(LocaleController.isRTL ? Gravity.RIGHT : Gravity.LEFT);
						textView1.setTypeface(Theme.TYPEFACE_BOLD);
						linearLayout.addView(textView1, LayoutHelper.createLinear(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.LEFT | Gravity.TOP, 0, 24, 0, 0));

						textView1 = new TextView(getContext());
						textView1.setText(AndroidUtilities.replaceTags(LocaleController.getString("EmojiSuggestionsInfo", R.string.EmojiSuggestionsInfo)));
						textView1.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 15);
						textView1.setTextColor(getThemedColor(Theme.key_dialogTextBlack));
						textView1.setGravity(LocaleController.isRTL ? Gravity.RIGHT : Gravity.LEFT);
						linearLayout.addView(textView1, LayoutHelper.createLinear(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.LEFT | Gravity.TOP, 0, 11, 0, 0));

						textView1 = new TextView(getContext());
						textView1.setText(LocaleController.formatString("EmojiSuggestionsUrl", R.string.EmojiSuggestionsUrl, lastSearchAlias != null ? lastSearchAlias : lastSearchKeyboardLanguage));
						textView1.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 15);
						textView1.setTextColor(getThemedColor(Theme.key_dialogTextLink));
						textView1.setGravity(LocaleController.isRTL ? Gravity.RIGHT : Gravity.LEFT);
						linearLayout.addView(textView1, LayoutHelper.createLinear(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.LEFT | Gravity.TOP, 0, 18, 0, 16));
						textView1.setOnClickListener(v1 -> {
							...
							TLRPC.TLMessagesGetEmojiURL req = new TLRPC.TLMessagesGetEmojiURL();
							req.langCode = lastSearchAlias != null ? lastSearchAlias : lastSearchKeyboardLanguage[0];
							int requestId = ConnectionsManager.getInstance(currentAccount).sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
								...
								if (response instanceof TLRPC.TLEmojiURL) {
									Browser.openUrl(getContext(), ((TLRPC.TLEmojiURL)response).url);
	...
	private class EmojiPagesAdapter extends PagerAdapter implements PagerSlidingTabStrip.IconTabProvider {
		...
		public CharSequence getPageTitle(int position) {
			return switch (position) {
				case 0 -> LocaleController.getString("Emoji", R.string.Emoji);
				case 1 -> LocaleController.getString("AccDescrGIFs", R.string.AccDescrGIFs);
				case 2 -> LocaleController.getString("AccDescrStickers", R.string.AccDescrStickers);
				default -> null;
			};
		}
	...
	private class GifAdapter extends RecyclerListView.SelectionAdapter {
		...
		private void searchBotUser() {
			...
			searchingUser = true;
			TLRPC.TLContactsResolveUsername req = new TLRPC.TLContactsResolveUsername();
			req.username = MessagesController.getInstance(currentAccount).gifSearchBot;
			ConnectionsManager.getInstance(currentAccount).sendRequest(req, (response, error) -> {
				if (response != null) {
					AndroidUtilities.runOnUIThread(() -> {
						TLRPC.TLContactsResolvedPeer res = (TLRPC.TLContactsResolvedPeer)response;
		...
		public void search(String text) {
			...
			if (!TextUtils.isEmpty(lastSearchImageString)) {
				AndroidUtilities.runOnUIThread(searchRunnable = () -> search(text, "", true), 300);
			}
		}
		...
		protected void search(final String query, final String offset, boolean searchUser, boolean isEmoji, boolean cache) {
			...
			if (cache) {
				...
				TLRPC.TLMessagesGetInlineBotResults req = new TLRPC.TLMessagesGetInlineBotResults();
				req.query = query == null ? "" : query;
				req.bot = MessagesController.getInstance(currentAccount).getInputUser(bot);
				req.offset = offset;
				req.peer = new TLRPC.TLInputPeerEmpty();
		...
		private void processResponse(final String query, final String offset, boolean searchUser, boolean isEmoji, boolean cache, String key, TLObject response) {
			...
			reqId = 0;
			if (cache && (!(response instanceof TLRPC.TLMessagesBotResults) || ((TLRPC.TLMessagesBotResults)response).results.isEmpty())) {
	...
	private class GifSearchPreloader {
		...
		private void processResponse(final String query, final String offset, boolean cache, String key, TLObject response) {
			...
			if (gifSearchAdapter.lastSearchIsEmoji && gifSearchAdapter.lastSearchImageString.equals(query)) {
				...
				else if (response instanceof TLRPC.TLMessagesBotResults && !gifCache.containsKey(key)) {
					gifCache.put(key, (TLRPC.TLMessagesBotResults)response);
				}
			}
		}
	}
	...
	private class GifLayoutManager extends ExtendedGridLayoutManager {
		...
		protected Size getSizeForItem(int i) {
			...
			List<TLRPC.DocumentAttribute> attributes = null;

			if (gifGridView.getAdapter() == gifAdapter) {
				if (i > gifAdapter.recentItemsCount) {
					TLRPC.BotInlineResult result = gifAdapter.results.get(i - gifAdapter.recentItemsCount - 1);
					document = TLRPCExtensions.getDocument(result);
					var content = TLRPCExtensions.getContent(result);
					var thumb = TLRPCExtensions.getThumb(result);

					if (document instanceof TLRPC.TLDocument tlDocument) {
						attributes = tlDocument.attributes;
					}
		...
		public Size getSizeForItem(TLRPC.Document document) {
			var attributers = document instanceof TLRPC.TLDocument tlDocument ? tlDocument.attributes : null;
			return getSizeForItem(document, attributers);
		}
	...
	private class StickersSearchGridAdapter extends RecyclerListView.SelectionAdapter {
		...
		private final Runnable searchRunnable = new Runnable() {
			...
			public void run() {
				...
				Map<String, List<TLRPC.Document>> allStickers = MediaDataController.getInstance(currentAccount).getAllStickers();
				if (searchQuery.length() <= 14) {
					...
					List<TLRPC.Document> newStickers = allStickers.get(emoji.toString());
				...
				if (!allStickers.isEmpty() && searchQuery.length() > 1) {
					String[] newLanguage = AndroidUtilities.getCurrentKeyboardLanguage();
					if (!Arrays.equals(lastSearchKeyboardLanguage, newLanguage)) {
						MediaDataController.getInstance(currentAccount).fetchNewEmojiKeywords(newLanguage);
					}
					...
					MediaDataController.getInstance(currentAccount).getEmojiSuggestions(lastSearchKeyboardLanguage, searchQuery, false, (param, alias) -> {
						...
						for (int a = 0, size = param.size(); a < size; a++) {
							...
							List<TLRPC.Document> newStickers = allStickers.get(emoji);
							if (newStickers != null && !newStickers.isEmpty()) {
								clear();
				...
				final TLRPC.TLMessagesSearchStickerSets req = new TLRPC.TLMessagesSearchStickerSets();
				req.q = searchQuery;
		...
		public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
			...
			switch (viewType) {
				...
				case 2:
					...
					break;
				case 3:
					view = new FeaturedStickerSetInfoCell(context, 17, false, true);
				...
				if (a == -1) {
					...
					if (idx < localCount) {
						...
						if (idx < emojiCount) {
							...
							documents = TLRPCExtensions.getCovers(set);
							pack = set;
						}
					}
				}
