...
public class InviteMembersBottomSheet extends UsersAlertBase implements NotificationCenter.NotificationCenterDelegate {
	private final LongSparseArray<TLObject> ignoreUsers;
	...
	private final View.OnClickListener spanClickListener = new View.OnClickListener() {
		@Override
	...
	public InviteMembersBottomSheet(Context context, int account, LongSparseArray<TLObject> ignoreUsers, long chatId, BaseFragment parentFragment, Theme.ResourcesProvider resourcesProvider) {
		...
		var arrayList = ContactsController.getInstance(account).contacts;

		for (int a = 0; a < arrayList.size(); a++) {
			User user = MessagesController.getInstance(currentAccount).getUser(arrayList.get(a).userId);
			...
			if (user instanceof TLRPC.TLUser tlUser) {
				if (tlUser.isSelf || tlUser.deleted) {
					continue;
				}
			}
			...
			if (listView.getAdapter() == searchAdapter) {
				...
				if (position == copyLinkRow) {
					...
					if (chat != null && !TextUtils.isEmpty(chat.username)) {
						link = String.format(Locale.getDefault(), "https://%s/%s", ApplicationLoader.applicationContext.getString(R.string.domain), chat.username);
					}
					else if (chatInfo != null && chatInfo.exportedInvite != null) {
						link = TLRPCExtensions.getLink(chatInfo.exportedInvite);
					}
					else {
						generateLink();
					}
		...
		floatingButton.setOnClickListener(v -> {
			...
			if (dialogsDelegate != null) {
				...
				if (selectedContacts.size() > 5) {
					...
					if (index >= 0) {
						spannableStringBuilder.setSpan(new TypefaceSpan(Theme.TYPEFACE_BOLD), index, index + countString.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
					}
	...
	private void onAddToGroupDone(int i) {
		...
		for (int a = 0; a < selectedContacts.size(); a++) {
			User user = MessagesController.getInstance(currentAccount).getUser(selectedContacts.keyAt(a));
			result.add(user);
		}
		if (delegate != null) {
		...
		if (spanEnter != enter) {
			...
			if (animated) {
				...
				if (!spanEnter && dialogsDelegate == null) {
					...
					currentDoneButtonAnimation.playTogether(ObjectAnimator.ofFloat(floatingButton, View.SCALE_X, 0.0f), ObjectAnimator.ofFloat(floatingButton, View.SCALE_Y, 0.0f), ObjectAnimator.ofFloat(floatingButton, View.ALPHA, 0.0f));
					...
					currentDoneButtonAnimation.playTogether(ObjectAnimator.ofFloat(floatingButton, View.SCALE_X, 1.0f), ObjectAnimator.ofFloat(floatingButton, View.SCALE_Y, 1.0f), ObjectAnimator.ofFloat(floatingButton, View.ALPHA, 1.0f));
					currentDoneButtonAnimation.setDuration(180);
					currentDoneButtonAnimation.start();
				}
			}
		...
		public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
			...
			switch (viewType) {
				...
				case 1:
					...
					manageChatTextCell.setColors(ResourcesCompat.getColor(context.getResources(), R.color.brand, null), ResourcesCompat.getColor(context.getResources(), R.color.brand, null));
		...
		public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
			switch (holder.getItemViewType()) {
				...
				case 3:
					...
					long oldId;
					if (oldObject instanceof User) {
						oldId = ((User)oldObject).id;
					}
	...
	private class SearchAdapter extends RecyclerListView.SelectionAdapter {
		...
		public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
			...
			View view = switch (viewType) {
				case 1 -> new GroupCreateUserCell(context, 1, 0, false);
				case 2 -> new View(context) {
					@Override
					protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
				...
				case 4 -> new View(context);
				default -> new GroupCreateSectionCell(context);
			};
			return new RecyclerListView.Holder(view);
		}
		...
		public void searchDialogs(final String query) {
			...
			searchAdapterHelper.queryServerSearch(null, true, false, false, false, false, 0, 0, 0, null);
			notifyDataSetChanged();
			...
			if (!TextUtils.isEmpty(query)) {
				...
				Utilities.searchQueue.postRunnable(searchRunnable = () -> AndroidUtilities.runOnUIThread(() -> {
					searchAdapterHelper.queryServerSearch(query, true, dialogsDelegate != null, true, dialogsDelegate != null, false, 0, 0, 0, null);
					Utilities.searchQueue.postRunnable(searchRunnable = () -> {
						...
						for (int a = 0; a < contacts.size(); a++) {
							...
							if (object instanceof User user) {
								name = ContactsController.formatName(user.firstName, user.lastName).toLowerCase();
	...
	private class SpansContainer extends ViewGroup {
		...
		private final int animationIndex = -1;
		...
		return new ContainerView(context) {
			...
			protected boolean drawChild(@NonNull Canvas canvas, View child, long drawingTime) {
				if (child == spansScrollView) {
					...
					canvas.drawColor(ColorUtils.setAlphaComponent(context.getColor(R.color.background), (int)(255 * spansEnterProgress)));
