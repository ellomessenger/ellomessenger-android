...
public class TableLayout extends View {
	...
	private final int itemPaddingTop = AndroidUtilities.dp(7);
	private final int itemPaddingLeft = AndroidUtilities.dp(8);
	private boolean drawLines;
	private boolean isStriped;
	private boolean isRtl;
	private final ArrayList<Child> cellsToFixHeight = new ArrayList<>();
	private final ArrayList<Point> rowSpans = new ArrayList<>();
	...
	public class Child {
		...
		private TLRPC.TLPageTableCell cell;
		...
		public void measure(int width, int height, boolean first) {
			...
			if (cell != null) {
				...
				if (textLayout != null) {
					...
					if (!first && (lineCount > 1 || lineCount > 0 && (cell.alignCenter || cell.alignRight))) {
					...
					if (textLeft != 0) {
						...
						if (cell.alignRight) {
							...
							textX += Math.round((float)(measuredWidth - textWidth) / 2);
						}
						else {
	...
	public interface TableLayoutDelegate {
		ArticleViewer.DrawingText createTextLayout(TLRPC.TLPageTableCell cell, int maxWidth);
	...
	private final TableLayoutDelegate delegate;

	private final ArrayList<Child> childrens = new ArrayList<>();
	...
	public void addChild(TLRPC.TLPageTableCell cell, int x, int y, int colspan) {
		if (colspan == 0) {
			colspan = 1;
		}
		Child child = new Child(childrens.size());
	...
	static Alignment getAlignment(int gravity, boolean horizontal) {
		...
		return switch (flags) {
			...
			case (AXIS_SPECIFIED | AXIS_PULL_BEFORE | RELATIVE_LAYOUT_DIRECTION) -> START;
			case (AXIS_SPECIFIED | AXIS_PULL_AFTER | RELATIVE_LAYOUT_DIRECTION) -> END;
	...
	final class Axis {
		...
		private Arc[] topologicalSort(final Arc[] arcs) {
			return new Object() {
				final Arc[] result = new Arc[arcs.length];
				int cursor = result.length - 1;
				final Arc[][] arcsByVertex = groupArcsByFirstVertex(arcs);
				final int[] visited = new int[getCount() + 1];
		...
		public int getMeasure(int measureSpec) {
			...
			return switch (mode) {
				case MeasureSpec.UNSPECIFIED -> getMeasure(0, MAX_SIZE);
				case EXACTLY -> getMeasure(size, size);
				case MeasureSpec.AT_MOST -> getMeasure(0, size);
				default -> 0;
			};
		}
