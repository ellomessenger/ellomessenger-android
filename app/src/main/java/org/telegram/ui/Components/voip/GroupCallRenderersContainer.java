...
import static org.telegram.ui.group.GroupCallActivity.TRANSITION_DURATION;
import static org.telegram.ui.group.GroupCallActivity.isLandscapeMode;
...
public class GroupCallRenderersContainer extends FrameLayout {
	...
	private final LongSparseIntArray attachedPeerIds = new LongSparseIntArray();
	int animationIndex;
	...
	private final boolean animateSpeakingOnNextDraw = true;
	private boolean drawRenderersOnly;
	private boolean drawFirst;
	private boolean notDrawRenderers;
	...
	public GroupCallRenderersContainer(@NonNull Context context, RecyclerView listView, RecyclerView fullscreenListView, ArrayList<GroupCallMiniTextureView> attachedRenderers, ChatObject.Call call, GroupCallActivity groupCallActivity) {
		...
		pinDrawable = new CrossOutDrawable(context, R.drawable.msg_pin_filled, Color.WHITE);
		...
		pinTextView.setTypeface(Theme.TYPEFACE_BOLD);
		...
		unpinTextView.setTypeface(Theme.TYPEFACE_BOLD);
	...
	private boolean isRtmpStream() {
		return call != null && call.call instanceof TLRPC.TLGroupCall groupCall && groupCall.rtmpStream;
	}
	...
	protected boolean drawChild(@NonNull Canvas canvas, View child, long drawingTime) {
		...
		if (child instanceof GroupCallMiniTextureView textureView) {
			if (textureView == fullscreenTextureView || textureView == outFullscreenTextureView || notDrawRenderers || textureView.drawFirst) {
				return true;
			}
	...
	protected void dispatchDraw(@NonNull Canvas canvas) {
		if (GroupCallActivity.isTabletMode) {
			drawRenderersOnly = true;
			super.dispatchDraw(canvas);
			drawRenderersOnly = false;
		}
		...
		if (GroupCallActivity.isTabletMode) {
			notDrawRenderers = true;
			super.dispatchDraw(canvas);
			notDrawRenderers = false;
		}
		else {
			super.dispatchDraw(canvas);
		}
...
//        }
		...
		if (oldInFullscreen != inFullscreenMode) {
			...
			fullscreenTextureView.textureView.syncOrRunAnimation(fullscreenAnimator);
		}
		...
		if (tapGesture && ev.getActionMasked() == MotionEvent.ACTION_UP && System.currentTimeMillis() - tapTime < 200) {
			...
			if (showSpeakingMembersToast) {
				...
				if (call != null && AndroidUtilities.rectTmp.contains(ev.getX(), ev.getY())) {
					...
					if (!found) {
						var participant = call.participants.get(speakingToastPeerId);
	...
	private void animateSwipeToBack(boolean aplay) {
		if (swipeToBackGesture) {
			...
			swipeToBackAnimator.setInterpolator(CubicBezierInterpolator.DEFAULT);
			if (fullscreenTextureView != null) {
				fullscreenTextureView.textureView.syncOrRunAnimation(swipeToBackAnimator);
			}
			else {
	...
	public void setAmplitude(TLRPC.TLGroupCallParticipant participant, float v) {
		for (int i = 0; i < attachedRenderers.size(); i++) {
	...
	public void setVisibleParticipant(boolean animated) {
		...
		for (int i = 0; i < call.currentSpeakingPeers.size(); i++) {
			...
			var participant = call.currentSpeakingPeers.get(key);
			if (participant.isSelf || participant.mutedByYou || MessageObject.getPeerId(fullscreenParticipant.participant.peer) == MessageObject.getPeerId(participant.peer)) {
				continue;
			}
			...
			if (newSpeaking) {
				...
				if (speakingIndex < 3) {
					var user = peerId > 0 ? MessagesController.getInstance(currenAccount).getUser(peerId) : null;
	...
	public boolean isVisible(TLRPC.TLGroupCallParticipant participant) {
		long peerId = MessageObject.getPeerId(participant.peer);
