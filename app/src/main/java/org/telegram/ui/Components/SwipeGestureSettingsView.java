...
public class SwipeGestureSettingsView extends FrameLayout {
	...
	public SwipeGestureSettingsView(Context context, int currentAccount) {
		...
		strings[SWIPE_GESTURE_PIN] = context.getString(R.string.SwipeSettingsPin);
		strings[SWIPE_GESTURE_READ] = context.getString(R.string.SwipeSettingsRead);
		strings[SWIPE_GESTURE_ARCHIVE] = context.getString(R.string.SwipeSettingsArchive);
		strings[SWIPE_GESTURE_MUTE] = context.getString(R.string.SwipeSettingsMute);
		strings[SWIPE_GESTURE_DELETE] = context.getString(R.string.SwipeSettingsDelete);
		strings[SWIPE_GESTURE_FOLDERS] = context.getString(R.string.SwipeSettingsFolders);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (currentColorKey == null) {
			...
			colorProgress = 1f;
			color = ColorUtils.blendARGB(getContext().getColor(R.color.background), Theme.getColor(currentColorKey), 0.9f);
			fromColor = color;
		}
		else if (!backgroundKeys[picker.getValue()].equals(currentColorKey)) {
			fromColor = ColorUtils.blendARGB(fromColor, ColorUtils.blendARGB(getContext().getColor(R.color.background), Theme.getColor(currentColorKey), 0.9f), colorProgress);
			colorProgress = 0;
			currentColorKey = backgroundKeys[picker.getValue()];
		}
		...
		color = ColorUtils.blendARGB(fromColor, ColorUtils.blendARGB(getContext().getColor(R.color.background), Theme.getColor(currentColorKey), 0.9f), colorProgress);

		filledPaint.setColor(color);
		canvas.drawRoundRect(rect, AndroidUtilities.dp(6), AndroidUtilities.dp(6), filledPaint);

		filledPaint.setColor(getContext().getColor(R.color.background));
...
//
	...
	public void updateIconColor(int i) {
		if (icons[i] != null) {
			int backgroundColor = ColorUtils.blendARGB(getContext().getColor(R.color.background), getContext().getColor(R.color.brand), 0.9f);
			int iconColor = getContext().getColor(R.color.white);
