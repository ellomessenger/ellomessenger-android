...
public class RadialProgress2 {
	...
	private final Paint miniProgressBackgroundPaint;
	private final Paint overlayPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
	private final Paint circlePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
	private final Paint circleMiniPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
	private final MediaActionDrawable mediaActionDrawable;
	private final MediaActionDrawable miniMediaActionDrawable;
	private final ImageReceiver overlayImageView;
	public boolean drawAsRoundedRect = false;
	...
	public void draw(Canvas canvas) {
		...
		if (overlayImageView.hasBitmapImage()) {
			...
			overlayImageView.setImageCoordinates(centerX - circleRadius, centerY - circleRadius, circleRadius * 2, circleRadius * 2);
		}
		...
		if (drawCircle && drawBackground) {
			if ((drawMiniIcon || circleCrossfadeColorKey != null) && miniDrawCanvas != null) {
				if (drawAsRoundedRect) {
					miniDrawCanvas.drawRoundRect(centerX - circleRadius, centerY - circleRadius, centerX + circleRadius, centerY + circleRadius, cornerRadius, cornerRadius, circleMiniPaint);
				}
				else {
					miniDrawCanvas.drawCircle(centerX, centerY, circleRadius, circleMiniPaint);
				}
				...
				if (currentIcon != MediaActionDrawable.ICON_NONE || wholeAlpha != 0) {
					if (backgroundStroke != 0) {
						float r = (circleRadius - AndroidUtilities.dp(3.5f));

						if (drawAsRoundedRect) {
							canvas.drawRoundRect(centerX - r, centerY - r, centerX + r, centerY + r, circleRadius, circleColor, circlePaint);
						}
						else {
						...
						if (drawAsRoundedRect) {
							canvas.drawRoundRect(centerX - circleRadius, centerY - circleRadius, centerX + circleRadius, centerY + circleRadius, cornerRadius, cornerRadius, circlePaint);
						}
						else {
		...
		if (overlayImageView.hasBitmapImage()) {
			...
			if ((drawMiniIcon || circleCrossfadeColorKey != null) && miniDrawCanvas != null) {
				...
				if (drawAsRoundedRect) {
					miniDrawCanvas.drawRoundRect(centerX - circleRadius, centerY - circleRadius, centerX + circleRadius, centerY + circleRadius, cornerRadius, cornerRadius, overlayPaint);
				}
				else {
				...
				if (drawAsRoundedRect) {
					canvas.drawRoundRect(centerX - circleRadius, centerY - circleRadius, centerX + circleRadius, centerY + circleRadius, cornerRadius, cornerRadius, overlayPaint);
				}
				else {
		...
		if ((drawMiniIcon || circleCrossfadeColorKey != null)) {
			...
			if (miniDrawCanvas != null) {
				centerX = AndroidUtilities.dp(18 + size + offset);
				centerY = AndroidUtilities.dp(18 + size + offset);
				float radius = AndroidUtilities.dp(halfSize + 1) * alpha * miniIconScale;

				if (drawAsRoundedRect) {
					miniDrawCanvas.drawRoundRect(centerX - radius, centerY - radius, centerX + radius, centerY + radius, cornerRadius, cornerRadius, Theme.checkboxSquare_eraserPaint);
				}
				else {
					miniDrawCanvas.drawCircle(centerX, centerY, radius, Theme.checkboxSquare_eraserPaint);
				}
				...
				if (drawAsRoundedRect) {
					canvas.drawRoundRect(cx - radius, cy - radius, cx + radius, cy + radius, cornerRadius, cornerRadius, miniProgressBackgroundPaint);
				}
				else {
					canvas.drawCircle(cx, cy, radius, miniProgressBackgroundPaint);
				}
			...
			float radius = AndroidUtilities.dp(halfSize) * alpha + AndroidUtilities.dp(1) * (1.0f - circleCheckProgress);

			canvas.drawCircle(cx, cy, radius, circleMiniPaint);
