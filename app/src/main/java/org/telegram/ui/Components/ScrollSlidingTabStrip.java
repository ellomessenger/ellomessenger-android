...
public class ScrollSlidingTabStrip extends HorizontalScrollView {
	...
	private final LinearLayout.LayoutParams defaultExpandLayoutParams;
	private final LinearLayout tabsContainer;
	private ScrollSlidingTabStripDelegate delegate;
	private HashMap<String, View> tabTypes = new HashMap<>();
	private HashMap<String, View> prevTypes = new HashMap<>();
	private final SparseArray<View> futureTabsPositions = new SparseArray<>();
	...
	private final float stickerTabExpandedWidth = AndroidUtilities.dp(EXPANDED_WIDTH);
	private final float stickerTabWidth = AndroidUtilities.dp(StickerTabView.SMALL_WIDTH);
			...
			if (child instanceof StickerTabView) {
				...
				if (tabView.type == StickerTabView.EMOJI_TYPE) {
					...
					if (!tabView.inited && tabView.svgThumb == null && sticker != null) {
						tabView.svgThumb = DocumentObject.getSvgThumb(sticker, ResourcesCompat.getColor(getContext().getResources(), R.color.light_background, null), 0.2f);
					}
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (!(isInEditMode() || tabCount == 0) && indicatorHeight >= 0) {
			...
			if (leftTab != null && rightTab != null) {
				cx = AndroidUtilities.lerp(leftTab.getLeft() + leftTab.getTranslationX() + AndroidUtilities.lerp(AndroidUtilities.dp(StickerTabView.SMALL_WIDTH), AndroidUtilities.dp(EXPANDED_WIDTH), expandProgress) / 2f, rightTab.getLeft() + rightTab.getTranslationX() + AndroidUtilities.lerp(AndroidUtilities.dp(StickerTabView.SMALL_WIDTH), AndroidUtilities.dp(EXPANDED_WIDTH), expandProgress) / 2f, position - floorPosition);
				textWidth = AndroidUtilities.lerp((leftTab instanceof StickerTabView ? ((StickerTabView)leftTab).getTextWidth() : 0), (rightTab instanceof StickerTabView ? ((StickerTabView)rightTab).getTextWidth() : 0), position - floorPosition);
			}
			else {
				if (leftTab != null) {
	...
	private boolean canSwap(int p) {
		...
		return child instanceof StickerTabView && ((StickerTabView)child).type == StickerTabView.STICKER_TYPE && !((StickerTabView)child).isChatSticker;
	}
