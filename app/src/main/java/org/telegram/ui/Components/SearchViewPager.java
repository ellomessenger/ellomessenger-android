...
public class SearchViewPager extends ViewPagerFixed implements FilteredSearchView.UiCallback {
	...
	private final RecyclerItemsEnterAnimator itemsEnterAnimator;
	private boolean attached;

	private NumberTextView selectedMessagesCountTextView;
	private boolean isActionModeShowed;
	private final HashMap<FilteredSearchView.MessageHashId, MessageObject> selectedFiles = new HashMap<>();
	...
	private final FilteredSearchView noMediaFiltersSearchView;
	...
	public SearchViewPager(Context context, DialogsActivity fragment, int type, int initialDialogsType, int folderId, ChatPreviewDelegate chatPreviewDelegate) {
		...
		searchListView = new BlurredRecyclerView(context) {
			...
			protected void dispatchDraw(@NonNull Canvas canvas) {
				if (dialogsSearchAdapter != null && searchLayoutManager != null && dialogsSearchAdapter.showMoreAnimation) {
					canvas.save();
					invalidate();
		...
		emptyView.title.setText(context.getString(R.string.NoResult));
	...
	private void showActionMode(boolean show) {
		...
		if (show && !parent.getActionBar().actionModeIsExist(actionModeTag)) {
			ActionBarMenu actionMode = parent.getActionBar().createActionMode(actionModeTag);

			selectedMessagesCountTextView = new NumberTextView(actionMode.getContext());
			selectedMessagesCountTextView.setTextSize(18);
			selectedMessagesCountTextView.setTypeface(Theme.TYPEFACE_BOLD);
			selectedMessagesCountTextView.setTextColor(getContext().getColor(R.color.text));
			...
			gotoItem = actionMode.addItemWithWidth(gotoItemId, R.drawable.msg_message, AndroidUtilities.dp(54), getContext().getString(R.string.AccDescrGoToMessage));
			forwardItem = actionMode.addItemWithWidth(forwardItemId, R.drawable.msg_forward, AndroidUtilities.dp(54), getContext().getString(R.string.Forward));
			deleteItem = actionMode.addItemWithWidth(deleteItemId, R.drawable.msg_delete, AndroidUtilities.dp(54), getContext().getString(R.string.Delete));
		}
	...
	public void onActionBarItemClick(int id) {
		if (id == deleteItemId) {
			...
			spannableStringBuilder.append(AndroidUtilities.replaceTags(LocaleController.formatPluralString("RemoveDocumentsMessage", selectedFiles.size()))).append("\n\n").append(getContext().getString(R.string.RemoveDocumentsAlertMessage));

			builder.setMessage(spannableStringBuilder);
			builder.setNegativeButton(getContext().getString(R.string.Cancel), (dialogInterface, i) -> dialogInterface.dismiss());
			builder.setPositiveButton(getContext().getString(R.string.Delete), (dialogInterface, i) -> {
			...
			if (button != null) {
				button.setTextColor(getContext().getColor(R.color.purple));
			}
		}
			...
			fragment.setDelegate((fragment1, dids, message, param) -> {
				...
				if (dids.size() > 1 || dids.get(0) == AccountInstance.getInstance(currentAccount).getUserConfig().getClientUserId() || message != null) {
					for (int a = 0; a < dids.size(); a++) {
						long did = dids.get(a);
						if (message != null) {
							AccountInstance.getInstance(currentAccount).getSendMessagesHelper().sendMessage(message.toString(), did, null, null, null, true, null, null, null, true, 0, null, false, false, null);
						}
	...
	protected void onItemSelected(View currentPage, View oldPage, int position, int oldPosition) {
		if (position == 0) {
			...
			boolean update = oldPosition == 0 && noMediaFiltersSearchView.getVisibility() != View.VISIBLE;
	...
	public void messagesDeleted(long channelId, List<Integer> markAsDeletedMessages) {
		int n = viewsByType.size();
		for (int i = 0; i < n; i++) {
			View v = viewsByType.valueAt(i);
			if (v instanceof FilteredSearchView) {
	...
	private class ViewPagerAdapter extends ViewPagerFixed.Adapter {
		...
		public String getItemTitle(int position) {
			if (items.get(position).type == DIALOGS_TYPE) {
				return getContext().getString(R.string.SearchAllChatsShort);
			}
			else if (items.get(position).type == DOWNLOADS_TYPE) {
				return getContext().getString(R.string.DownloadsTabs);
			}
			else {
