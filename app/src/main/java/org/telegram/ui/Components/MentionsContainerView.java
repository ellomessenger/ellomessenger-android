...
public class MentionsContainerView extends BlurredFrameLayout {
	...
	private final MentionsListView listView;
	private final LinearLayoutManager linearLayoutManager;
	private final ExtendedGridLayoutManager gridLayoutManager;
	private final PaddedListAdapter paddedAdapter;
	private final MentionsAdapter adapter;
	ChatActivity chatActivity;

	private float containerTop, containerBottom, containerPadding, listViewPadding;

	public MentionsContainerView(@NonNull Context context, long dialogId, int threadMessageId, ChatActivity chatActivity) {
		...
		listView = new MentionsListView(context);
		...
		adapter = new MentionsAdapter(context, dialogId, threadMessageId, new MentionsAdapter.MentionsAdapterDelegate() {
			@Override
	...
	public void dispatchDraw(Canvas canvas) {
		...
		paint.setColor(color != null ? color : ApplicationLoader.applicationContext.getColor(R.color.background));
	...
	private final Runnable updateVisibilityRunnable = () -> updateListViewTranslation(!shown, true);
...
//                adapter.clear(true);
	...
	public class MentionsListView extends RecyclerListView {
		...
		public MentionsListView(Context context) {
			super(context);
		...
		public boolean onInterceptTouchEvent(@NonNull MotionEvent event) {
			...
			boolean result = !isScrolling && ContentPreviewViewer.getInstance().onInterceptTouchEvent(event, listView, 0, null);
