...
public class ThemeSmallPreviewView extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
	...
	public ThemeSmallPreviewView(Context context, int currentAccount, int currentType) {
		...
		backupImageView.imageReceiver.setCrossfadeWithOldImage(true);
		backupImageView.imageReceiver.setAllowStartLottieAnimation(false);
		backupImageView.imageReceiver.setAutoRepeat(0);
	...
	public void setItem(ChatThemeBottomSheet.ChatThemeItem item, boolean animated) {
		...
		if (itemChanged) {
			Drawable thumb = null;
			if (document != null) {
				thumb = DocumentObject.getSvgThumb(document, ResourcesCompat.getColor(getContext().getResources(), R.color.light_background, null), 0.2f);
			}
			if (thumb == null) {
		...
		if (!animated) {
			...
			backupImageView.setScaleY(1f);
			AndroidUtilities.cancelRunOnUIThread(animationCancelRunnable);
			if (backupImageView.imageReceiver.getLottieAnimation() != null) {
				backupImageView.imageReceiver.getLottieAnimation().stop();
				backupImageView.imageReceiver.getLottieAnimation().setCurrentFrame(0, false);
			}
		}
	...
	public void updatePreviewBackground(ThemeDrawable themeDrawable) {
		...
		int strokeColor = chatThemeItem.chatTheme.showAsDefaultStub ? getThemedColor(Theme.key_featuredStickers_addButton) : themeItem.outLineColor;
	...
	private StaticLayout getNoThemeStaticLayout() {
		...
		noThemeTextPaint.setTypeface(Theme.TYPEFACE_BOLD);
		textLayout = StaticLayoutEx.createStaticLayout2(LocaleController.getString("ChatNoTheme", R.string.ChatNoTheme), noThemeTextPaint, Layout.Alignment.ALIGN_CENTER, 1f, 0f, true, AndroidUtilities.dp(52), 3);
		return textLayout;
	}
	...
	public void playEmojiAnimation() {
		if (backupImageView.imageReceiver.getLottieAnimation() != null) {
			...
			if (!backupImageView.imageReceiver.getLottieAnimation().isRunning) {
				backupImageView.imageReceiver.getLottieAnimation().setCurrentFrame(0, true);
				backupImageView.imageReceiver.getLottieAnimation().start();
			}
