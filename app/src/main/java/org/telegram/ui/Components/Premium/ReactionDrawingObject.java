...
public class ReactionDrawingObject extends CarouselView.DrawingObject {
	...
	public void onAttachToWindow(View parentView, int i) {
		...
		if (i == 0) {
			...
			SvgHelper.SvgDrawable svgThumb = DocumentObject.getSvgThumb(reaction.activate_animation, ResourcesCompat.getColor(parentView.getContext().getResources(), R.color.light_background, null), 0.5f);
...
//
	...
	public void draw(Canvas canvas, float cX, float cY, float globalScale) {
		...
		rect.set((int)(cX - imageSize / 2f), (int)(cY - imageSize / 2f), (int)(cX + imageSize / 2f), (int)(cY + imageSize / 2f));

		imageReceiver.setImageCoordinates(cX - imageSize / 2f, cY - imageSize / 2f, imageSize, imageSize);
		actionReceiver.setImageCoordinates(cX - imageSize / 2f, cY - imageSize / 2f, imageSize, imageSize);
		...
		if (selected || selectedProgress != 0) {
			effectImageReceiver.setImageCoordinates(cX - effectSize / 2f, cY - effectSize / 2f, effectSize, effectSize);
	...
	public void set(TL_availableReaction reaction) {
