...
//			}
	...
	private void setVideo() {
		var premiumPromo = MediaDataController.getInstance(currentAccount).getPremiumPromo();
		String typeString = PremiumPreviewFragment.featureTypeToServerString(type);
		if (premiumPromo != null) {
			int index = -1;
			for (int i = 0; i < premiumPromo.videoSections.size(); i++) {
				if (premiumPromo.videoSections.get(i).equals(typeString)) {
					index = i;
					break;
				}
			}
			if (index >= 0) {
				...
				if (document instanceof TLRPC.TLDocument tlDocument) {
					for (int i = 0; i < tlDocument.thumbs.size(); i++) {
						if (tlDocument.thumbs.get(i) instanceof TLRPC.TLPhotoStrippedSize) {
							roundedBitmapDrawable = RoundedBitmapDrawableFactory.create(getResources(), ImageLoader.getStrippedPhotoBitmap(tlDocument.thumbs.get(i).bytes, "b"));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (fromTop) {
			AndroidUtilities.rectTmp.set(horizontalPadding, 0, measuredWidth - horizontalPadding, (float)size);
		}
		else {
			AndroidUtilities.rectTmp.set(horizontalPadding, measuredHeight - (float)size, measuredWidth - horizontalPadding, measuredHeight);
		}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (fromTop) {
			AndroidUtilities.rectTmp.set(horizontalPadding, -roundRadius, getMeasuredWidth() - horizontalPadding, (float)size);
		}
		else {
			AndroidUtilities.rectTmp.set(horizontalPadding, getMeasuredHeight() - (float)size, getMeasuredWidth() - horizontalPadding, getMeasuredHeight() + roundRadius);
		}
	...
	protected void dispatchDraw(@NonNull Canvas canvas) {
		...
		if (fromTop) {
			AndroidUtilities.rectTmp.set(horizontalPadding, 0, getMeasuredWidth() - horizontalPadding, (float)size);
		}
		else {
			AndroidUtilities.rectTmp.set(horizontalPadding, getMeasuredHeight() - (float)size, getMeasuredWidth() - horizontalPadding, getMeasuredHeight());
		}
		...
		if (!firstFrameRendered) {
			imageReceiver.setImageCoordinates(AndroidUtilities.rectTmp.left, AndroidUtilities.rectTmp.top, AndroidUtilities.rectTmp.width(), AndroidUtilities.rectTmp.height());
