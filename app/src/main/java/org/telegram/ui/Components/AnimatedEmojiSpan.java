...
public class AnimatedEmojiSpan extends ReplacementSpan {
	...
	public AnimatedEmojiSpan(@NonNull TLRPC.Document document, @Nullable Paint.FontMetricsInt fontMetrics) {
		this(document.id, 1.2f, fontMetrics);
		this.document = document;
	}

	public AnimatedEmojiSpan(long documentId, @Nullable Paint.FontMetricsInt fontMetrics) {
		this(documentId, 1.2f, fontMetrics);
	}

	public AnimatedEmojiSpan(long documentId, float scale, @Nullable Paint.FontMetricsInt fontMetrics) {
	...
	public void replaceFontMetrics(@Nullable Paint.FontMetricsInt newMetrics, int newSize, int cacheType) {
		fontMetrics = newMetrics;
		size = newSize;
		this.cacheType = cacheType;
	}

	public void applyFontMetrics(@Nullable Paint.FontMetricsInt newMetrics, int cacheType) {
		fontMetrics = newMetrics;
		this.cacheType = cacheType;
	}
	...
	public static class AnimatedEmojiHolder implements InvalidateHolder {
		...
		public void prepareForBackgroundDraw(long updateTime) {
			...
			if (imageReceiver != null) {
				...
				imageReceiver.setImageCoordinates(drawableBounds);
...
//                drawable.setTime(0);
	...
	public static EmojiGroupedSpans update(int cacheType, View view, EmojiGroupedSpans prev, ArrayList<TextLayoutBlock> blockLayouts) {
		return update(cacheType, view, prev, blockLayouts, false);
	}

	public static EmojiGroupedSpans update(int cacheType, View view, EmojiGroupedSpans prev, ArrayList<TextLayoutBlock> blockLayouts, boolean clone) {
		return update(cacheType, view, false, prev, blockLayouts, clone);
	}

	public static EmojiGroupedSpans update(int cacheType, View view, boolean invalidateParent, EmojiGroupedSpans prev, ArrayList<TextLayoutBlock> blockLayouts) {
		return update(cacheType, view, invalidateParent, prev, blockLayouts, false);
	}

	public static EmojiGroupedSpans update(int cacheType, View view, boolean invalidateParent, EmojiGroupedSpans prev, ArrayList<TextLayoutBlock> blockLayouts, boolean clone) {
			...
			if (textLayout != null && textLayout.getText() instanceof Spanned spanned) {
				...
				for (int i = 0; spans != null && i < spans.length; ++i) {
					...
					for (int j = 0; j < prev.holders.size(); ++j) {
						if (prev.holders.get(j).span == span && prev.holders.get(j).layout == textLayout) {
		...
		for (int i = 0; i < prev.size(); ++i) {
			...
			if (d == null) {
				...
				for (AnimatedEmojiSpan span : spans) {
					if (span != null && span.getDocumentId() == documentId) {
						found = true;
						break;
					}
				}
	...
	public static void release(EmojiGroupedSpans spans) {
		if (spans == null) {
			return;
		}
		spans.release();
	...
	@Nullable
	public static CharSequence cloneSpans(@Nullable CharSequence text) {
		...
		for (CharacterStyle span : spans) {
			...
			if (span instanceof AnimatedEmojiSpan oldSpan) {
