...
public class LinkSpanDrawable<S extends CharacterStyle> {
	...
	public LinkSpanDrawable(S span, float touchX, float touchY) {
		this(span, touchX, touchY, true);
	}

	public LinkSpanDrawable(S span, float touchX, float touchY, boolean supportsLongPress) {
		mSpan = span;
		setColor(Theme.getColor(Theme.key_chat_linkSelectBackground));
		mTouchX = touchX;
		mTouchY = touchY;
	...
	public boolean draw(Canvas canvas) {
		...
		if (mBounds == null && mPathesCount > 0) {
			...
			mBounds = new Rect((int)AndroidUtilities.rectTmp.left, (int)AndroidUtilities.rectTmp.top, (int)AndroidUtilities.rectTmp.right, (int)AndroidUtilities.rectTmp.bottom);
		...
		float pressT = CubicBezierInterpolator.DEFAULT.getInterpolation(Math.min(1, (now - mStart) / (float)mDuration)), releaseT = mReleaseStart < 0 ? 0 : Math.min(1, Math.max(0, (now - mReleaseDelay - mReleaseStart) / (float)mReleaseDuration));
		float longPress;
		...
		public void removeLink(LinkSpanDrawable link, boolean animated) {
			...
			if (animated) {
				if (link.mReleaseStart < 0) {
					...
					final long now = SystemClock.elapsedRealtime();
					AndroidUtilities.runOnUIThread(() -> removeLink(link, false), Math.max(0, (link.mReleaseStart - now) + mReleaseDelay + mReleaseDuration));
				}
			}
			else {
				mLinks.remove(pair);
	...
	public static class LinksTextView extends TextView {
		...
		public LinksTextView(Context context, LinkCollector customLinkCollector) {
			super(context);
				...
				if (left <= x && left + textLayout.getLineWidth(line) >= x && y >= 0 && y <= textLayout.getHeight()) {
					...
					if (spans.length != 0 && !AndroidUtilities.isAccessibilityScreenReaderEnabled()) {
						...
						if (event.getAction() == MotionEvent.ACTION_DOWN) {
							pressedLink = new LinkSpanDrawable<ClickableSpan>(span, event.getX(), event.getY());
