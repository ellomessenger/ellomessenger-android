...
public class BotWebViewContainer extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
	...
	private final TextView webViewNotAvailableText;
	private final CellFlickerDrawable flickerDrawable = new CellFlickerDrawable();
	private final BackupImageView flickerView;
	...
	public BotWebViewContainer(@NonNull Context context, int backgroundColor) {
		super(context);
		...
		flickerView = new BackupImageView(context) {
			{
				imageReceiver = new ImageReceiver(this) {
					@Override
					public boolean setImageBitmapByKey(@Nullable Drawable drawable, @Nullable String key, int type, boolean memCache, int guid) {
			...
			protected void onDraw(@NonNull Canvas canvas) {
				if (isFlickeringCenter) {
					...
					if (drawable != null) {
						imageReceiver.setImageCoordinates(0, 0, getWidth(), drawable.getIntrinsicHeight() * ((float)getWidth() / drawable.getIntrinsicWidth()));
		...
		flickerView.imageReceiver.setAspectFit(true);
		...
		webViewNotAvailableText.setText(context.getString(R.string.BotWebViewNotAvailablePlaceholder));
	...
	private void onOpenUri(Uri uri, boolean suppressPopup) {
		...
		if (internal && !forceBrowser[0]) {
			...
			new AlertDialog.Builder(getContext()).setTitle(getContext().getString(R.string.OpenUrlTitle)).setMessage(LocaleController.formatString(R.string.OpenUrlAlert2, uri.toString())).setPositiveButton(getContext().getString(R.string.Open), (dialog, which) -> Browser.openUrl(getContext(), uri, true, false)).setNegativeButton(getContext().getString(R.string.Cancel), null).setOnDismissListener(dialog -> isRequestingPageOpen = false).show();
		}
	}
	...
	private void setPageLoaded(String url) {
		...
		set.playTogether(ObjectAnimator.ofFloat(webView, View.ALPHA, 1f), ObjectAnimator.ofFloat(flickerView, View.ALPHA, 0f));
	...
	public void didReceivedNotification(int id, int account, Object... args) {
		if (id == NotificationCenter.didSetNewTheme) {
			...
			onRequestPermissionsResult((int)args[0]);
		}
	}
	...
	private void onEventReceived(String eventType, String eventData) {
		...
		switch (eventType) {
			...
			case "web_app_request_phone": {
				...
				AlertDialog.Builder builder = new AlertDialog.Builder(getContext()).setTitle(getContext().getString(R.string.ShareYouPhoneNumberTitle)).setMessage(getContext().getString(R.string.AreYouSureShareMyContactInfoBot)).setPositiveButton(getContext().getString(R.string.ShareContact), (dialogInterface, i) -> {
					User currentUser = UserConfig.getInstance(currentAccount).getCurrentUser();
					if (currentUser != null) {
						try {
							notifyEvent("phone_requested", new JSONObject().put("phone_number", /* currentUser.phone */ currentUser.username));
						}
						catch (JSONException e) {
							FileLog.e(e);
						}
				...
				}).setNegativeButton(getContext().getString(R.string.Cancel), null).setOnDismissListener(dialog1 -> {
			...
			case "web_app_open_popup": {
				try {
					...
					AlertDialog.Builder builder = new AlertDialog.Builder(getContext()).setTitle(title).setMessage(message);
			...
			case "web_app_set_header_color": {
				try {
					JSONObject jsonObject = new JSONObject(eventData);
					delegate.onWebAppSetActionBarColor(Color.parseColor(jsonObject.optString("color", "#ffffff")) | 0xFF000000);
				}
				catch (JSONException | IllegalArgumentException e) {
					FileLog.e(e);
				}
...
//				}
			...
			case "web_app_open_tg_link": {
				try {
					...
					onOpenUri(Uri.parse(String.format(Locale.getDefault(), "https://%s/", ApplicationLoader.applicationContext.getString(R.string.domain)) + pathFull));
				}
				catch (JSONException e) {
					FileLog.e(e);
				}
	...
	public interface Delegate {
		 ...
		 */
		void onWebAppSetActionBarColor(int color);
	...
	public final static class PopupButton {
		...
		public PopupButton(JSONObject obj) throws JSONException {
			...
			switch (type) {
				...
				case "ok": {
					text = ApplicationLoader.applicationContext.getString(R.string.OK);
					break;
				}
				case "close": {
					text = ApplicationLoader.applicationContext.getString(R.string.Close);
					break;
				}
				case "cancel": {
					text = ApplicationLoader.applicationContext.getString(R.string.Cancel);
					break;
				}
				case "destructive": {
					textRequired = true;
