		...
		public StickerView(Context context) {
			...
			view = new View(context) {
				...
				public void draw(Canvas canvas) {
					super.draw(canvas);
					if (update) {
						SvgHelper.SvgDrawable svgThumb = DocumentObject.getSvgThumb(document, ResourcesCompat.getColor(getContext().getResources(), R.color.light_background, null), 0.5f);
					...
					centerImage.setImageCoordinates(size - smallImageSize - size * 0.02f + x, (size - smallImageSize) / 2f + y, smallImageSize, smallImageSize);
					centerImage.setAlpha(0.3f + 0.7f * animateImageProgress);
					centerImage.draw(canvas);

					if (effectProgress != 0) {
						effectImage.setImageCoordinates(x, y, size, size);
