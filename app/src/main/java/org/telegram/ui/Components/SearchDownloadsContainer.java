...
public class SearchDownloadsContainer extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
	...
	public void update(boolean animated) {
		...
		if (TextUtils.isEmpty(searchQuery) || isEmptyDownloads()) {
			...
			if (rowCount == 0) {
				...
				emptyView.title.setText(R.string.SearchEmptyViewDownloads);
				...
				AndroidUtilities.runOnUIThread(() -> {
					if (q.equals(lastQueryString)) {
						...
						if (rowCount == 0) {
							...
							emptyView.title.setText(R.string.NoResult);
							emptyView.subtitle.setVisibility(View.VISIBLE);
							emptyView.subtitle.setText(R.string.SearchEmptyViewFilteredSubtitle2);
						}
					}
				});
			}, sameQuery ? 0 : 300);
		...
		public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
			...
			if (type == 0) {
				...
				if (position == downloadingFilesHeader) {
					String header = getContext().getString(R.string.Downloading);
					if (graySectionCell.getText().equals(header)) {
						graySectionCell.setRightText(hasCurrentDownload ? getContext().getString(R.string.PauseAll) : getContext().getString(R.string.ResumeAll), hasCurrentDownload);
					}
					else {
						graySectionCell.setText(header, hasCurrentDownload ? getContext().getString(R.string.PauseAll) : getContext().getString(R.string.ResumeAll), view -> {
							for (int i = 0; i < currentLoadingFiles.size(); i++) {
					...
					graySectionCell.setText(getContext().getString(R.string.RecentlyDownloaded), getContext().getString(R.string.Settings), view -> showSettingsDialog());
				}
			}
			else {
				...
				if (messageObject != null) {
					...
					if (type == 1) {
						...
						view.setBackgroundColor(getContext().getColor(R.color.background));
	...
	private void showSettingsDialog() {
		...
		title.setText(R.string.DownloadedFiles);
		...
		description.setTextColor(context.getColor(R.color.hint));
		...
		buttonTextView.setText(context.getString(R.string.ManageDeviceStorage));

		buttonTextView.setTextColor(context.getColor(R.color.white));
		buttonTextView.setBackground(Theme.createSimpleSelectorRoundRectDrawable(AndroidUtilities.dp(6), context.getColor(R.color.brand), ColorUtils.setAlphaComponent(context.getColor(R.color.background), 120)));
		...
		buttonTextView2.setTypeface(Theme.TYPEFACE_BOLD);
		buttonTextView2.setText(context.getString(R.string.ClearDownloadsList));

		buttonTextView2.setTextColor(context.getColor(R.color.white));
		buttonTextView2.setBackground(Theme.createSimpleSelectorRoundRectDrawable(AndroidUtilities.dp(6), Color.TRANSPARENT, ColorUtils.setAlphaComponent(context.getColor(R.color.brand), 120)));
		...
		bottomSheet.show();
		AndroidUtilities.setLightStatusBar(bottomSheet.getWindow(), !AndroidUtilities.isDarkTheme());
		AndroidUtilities.setLightNavigationBar(bottomSheet.getWindow(), !AndroidUtilities.isDarkTheme());
