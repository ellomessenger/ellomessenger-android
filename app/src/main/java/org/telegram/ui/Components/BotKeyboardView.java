...
public class BotKeyboardView extends LinearLayout {
	...
	private TLRPC.TLReplyKeyboardMarkup botButtons;
	...
	public void updateColors() {
		...
		for (int i = 0; i < buttonViews.size(); i++) {
			...
			Drawable drawable = getContext().getDrawable(R.drawable.bot_button_background);
			buttonViews.get(i).setBackground(drawable);
	...
	public void setButtons(TLRPC.TLReplyKeyboardMarkup buttons) {
		botButtons = buttons;
		...
		if (buttons != null && botButtons.rows.size() != 0) {
			...
			for (int a = 0; a < buttons.rows.size(); a++) {
				TLRPC.TLKeyboardButtonRow row = buttons.rows.get(a);
				...
				for (int b = 0; b < row.buttons.size(); b++) {
					...
					textView.setBackground(getContext().getDrawable(R.drawable.bot_button_background));
					...
					textView.setText(Emoji.replaceEmoji(button.text, textView.getPaint().getFontMetricsInt(), false));
					layout.addView(frame, LayoutHelper.createLinear(0, LayoutHelper.MATCH_PARENT, weight, 0, 0, b != row.buttons.size() - 1 ? 10 : 0, 0));
					textView.setOnClickListener(v -> {
						for (int i = 0; i < buttonViews.size(); i++) {
							buttonViews.get(i).setSelected(v == buttonViews.get(i));
						}
					...
					if (button instanceof TLRPC.TLKeyboardButtonWebView || button instanceof TLRPC.TLKeyboardButtonSimpleWebView) {
