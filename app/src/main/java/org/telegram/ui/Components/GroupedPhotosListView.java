...
public class GroupedPhotosListView extends View implements GestureDetector.OnGestureListener {
	...
	private final ArrayList<ImageReceiver> unusedReceivers = new ArrayList<>();
	private final ArrayList<ImageReceiver> imagesToDraw = new ArrayList<>();
	public ArrayList<ImageLocation> currentPhotos = new ArrayList<>();
	private final ArrayList<Object> currentObjects = new ArrayList<>();
	...
	private final android.widget.Scroller scroll;
		...
		if (addRightIndex != Integer.MIN_VALUE) {
			...
			for (int a = addRightIndex; a < count; a++) {
				...
				if (x < width) {
					...
					receiver.setImageCoordinates(x, itemY, itemWidth, itemHeight);
					Object parent;
