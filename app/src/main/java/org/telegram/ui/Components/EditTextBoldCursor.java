...
public class EditTextBoldCursor extends EditTextEffects {
	...
	public void setTextWatchersSuppressed(boolean textWatchersSuppressed, boolean dispatchChanged) {
		if (isTextWatchersSuppressed == textWatchersSuppressed) {
			return;
		}
	...
	private void init() {
		...
		errorPaint.setTypeface(Theme.TYPEFACE_DEFAULT);
	...
	public void setLineColors(int color, int active, int error) {
		lineVisible = true;
		ResourcesCompat.getDrawable(getResources(), R.drawable.search_dark, null).getPadding(padding);
		setPadding(padding.left, padding.top, padding.right, padding.bottom);
		lineColor = color;
		activeLinePaint.setColor(active);
		errorLineColor = error;
		errorPaint.setColor(errorLineColor);
		invalidate();
	}
	...
	protected void onDraw(Canvas canvas) {
		if ((length() == 0 || transformHintToHeader) && hintLayout != null && (hintVisible || hintAlpha != 0)) {
			...
			if (supportRtlHint && LocaleController.isRTL) {
				...
				canvas.translate(getPaddingLeft() + left + getScrollX() + offset, lineY - hintLayout.getHeight() - AndroidUtilities.dp(7));
			}
			else {
				canvas.translate(getPaddingLeft() + left + getScrollX(), lineY - hintLayout.getHeight() - AndroidUtilities.dp2(7));
			}
		...
		if (lineVisible && lineColor != 0) {
			...
			int centerX = lastTouchX < 0 ? getMeasuredWidth() / 2 : lastTouchX, maxWidth = Math.max(centerX, getMeasuredWidth() - centerX) * 2;
			...
			if (lineActiveness > 0f) {
				...
				canvas.drawRect(getScrollX() + Math.max(0, centerX - activeLineWidth / 2), bottom - lineThickness, getScrollX() + Math.min(centerX + activeLineWidth / 2, getMeasuredWidth()), bottom, activeLinePaint);
			}
		}
	}
	...
	public ActionMode startActionMode(ActionMode.Callback callback) {
		if (windowView != null || attachedToWindow != null) {
