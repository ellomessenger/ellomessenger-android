...
public class VoIPFloatingLayout extends FrameLayout {
	...
	private final ValueAnimator.AnimatorUpdateListener mutedUpdateListener = valueAnimator -> {
	...
	private void setRelativePositionInternal(float xRelative, float yRelative, int width, int height, boolean animated) {
		...
		if (animated) {
			animate().setListener(null).cancel();
			animate().scaleX(1f).scaleY(1f).translationX(xPoint).translationY(yPoint).alpha(1f).setStartDelay(0).setDuration(150).setInterpolator(CubicBezierInterpolator.DEFAULT).start();
		}
		else {
			if (!alwaysFloating) {
				animate().setListener(null).cancel();
	...
	public void setFloatingMode(boolean show, boolean animated) {
		...
		if (show && !floatingMode) {
			...
			if (relativePositionToSetX >= 0) {
				setRelativePositionInternal(relativePositionToSetX, relativePositionToSetY, (int)(getMeasuredWidth() * FLOATING_MODE_SCALE), (int)(getMeasuredHeight() * FLOATING_MODE_SCALE), false);
			}
			floatingMode = false;
