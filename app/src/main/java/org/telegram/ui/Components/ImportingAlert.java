...
public class ImportingAlert extends BottomSheet implements NotificationCenter.NotificationCenterDelegate {
	...
	private final String stickersShortName;
	...
	public ImportingAlert(final Context context, String shortName, ChatActivity chatActivity) {
		...
		lineProgressView.setProgressColor(context.getColor(R.color.brand));
		lineProgressView.setBackColor(context.getColor(R.color.white));
		...
		for (int a = 0; a < 2; a++) {
			...
			importCountTextView[a].setTypeface(Theme.TYPEFACE_BOLD);
			importCountTextView[a].setTextColor(context.getColor(R.color.text));
			...
			infoTextView[a].setTextColor(context.getColor(R.color.dark_gray));
		...
		if (parentFragment != null) {
			...
			percentTextView.setText(String.format(Locale.getDefault(), "%d%%", importingHistory.uploadProgress));
			...
			percentTextView.setText(String.format(Locale.getDefault(), "%d%%", importingStickers.uploadProgress));
