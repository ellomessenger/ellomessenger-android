...
public class GroupCallPipAlertView extends LinearLayout implements StateListener, NotificationCenter.NotificationCenterDelegate {
	...
	public GroupCallPipAlertView(Context context, int account) {
		...
		groupInfoContainer = new FrameLayout(context) {
			...
			public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
				...
				if (service != null && ChatObject.isChannelOrGiga(service.getChat())) {
					info.addAction(new AccessibilityNodeInfo.AccessibilityAction(AccessibilityNodeInfo.ACTION_CLICK, context.getString(R.string.VoipChannelOpenVoiceChat)));
				}
				else {
					info.addAction(new AccessibilityNodeInfo.AccessibilityAction(AccessibilityNodeInfo.ACTION_CLICK, context.getString(R.string.VoipGroupOpenVoiceChat)));
				}
			}
		};
		...
		groupInfoContainer.setOnClickListener(view -> {
			if (VoIPService.getSharedInstance() != null) {
				Intent intent = new Intent(getContext(), LaunchActivity.class).setAction(VoIPPreNotificationService.VOIP_CHAT_ACTION);
		...
		leaveButton.setData(R.drawable.calls_decline, 0xffffffff, 0xFFCE4A4A, 0.3f, false, context.getString(R.string.VoipGroupLeave), false, false);
		leaveButton.setOnClickListener(v -> GroupCallActivity.onLeaveClick(getContext(), () -> GroupCallPip.updateVisibility(context), Settings.canDrawOverlays(context)));
	...
	private void updateButtons(boolean animated) {
		...
		if (bluetooth) {
			soundButton.setData(R.drawable.calls_bluetooth, Color.WHITE, 0, 0.1f, true, getContext().getString(R.string.VoipAudioRoutingBluetooth), false, animated);
		}
		else if (checked) {
			soundButton.setData(R.drawable.calls_speaker, Color.WHITE, 0, 0.3f, true, getContext().getString(R.string.VoipSpeaker), false, animated);
		}
		else {
			if (service.isHeadsetPlugged()) {
				soundButton.setData(R.drawable.calls_headphones, Color.WHITE, 0, 0.1f, true, getContext().getString(R.string.VoipAudioRoutingHeadset), false, animated);
			}
			else {
				soundButton.setData(R.drawable.calls_speaker, Color.WHITE, 0, 0.1f, true, getContext().getString(R.string.VoipSpeaker), false, animated);
			}
		}

		if (service.mutedByAdmin()) {
			muteButton.setData(R.drawable.calls_unmute, Color.WHITE, ColorUtils.setAlphaComponent(Color.WHITE, (int)(255 * 0.3f)), 0.1f, true, getContext().getString(R.string.VoipMutedByAdminShort), true, animated);
		}
		else {
			muteButton.setData(R.drawable.calls_unmute, Color.WHITE, ColorUtils.setAlphaComponent(Color.WHITE, (int)(255 * (service.isMicMute() ? 0.3f : 0.15f))), 0.1f, true, service.isMicMute() ? getContext().getString(R.string.VoipUnmute) : getContext().getString(R.string.VoipMute), service.isMicMute(), animated);
		}
		invalidate();
	}
