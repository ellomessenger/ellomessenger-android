...
public class AnimatedPhoneNumberEditText extends HintEditText {
	...
	private final HintFadeProperty hintFadeProperty = new HintFadeProperty();
	private final List<Float> hintAnimationValues = new ArrayList<>();
	private final List<SpringAnimation> hintAnimations = new ArrayList<>();
	...
	private void runHintAnimation(int length, boolean show, Runnable callback) {
		...
		for (int i = 0; i < length; i++) {
			...
			SpringAnimation springAnimation = new SpringAnimation(i, hintFadeProperty).setSpring(new SpringForce(finalValue * SPRING_MULTIPLIER).setStiffness(500).setDampingRatio(SpringForce.DAMPING_RATIO_NO_BOUNCY).setFinalPosition(finalValue * SPRING_MULTIPLIER)).setStartValue(startValue * SPRING_MULTIPLIER);
	...
	public void setNewText(String text) {
		if (oldLetters == null || letters == null || Objects.equals(oldText, text)) {
			return;
		}
