...
public class SenderSelectView extends View {
	...
	private final Paint menuPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
	...
	private void updateColors() {
		backgroundPaint.setColor(getContext().getColor(R.color.brand));
		menuPaint.setColor(getContext().getColor(R.color.white));
		selectorDrawable = Theme.createSimpleSelectorRoundRectDrawable(AndroidUtilities.dp(16), Color.TRANSPARENT, getContext().getColor(R.color.background));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		avatarImage.setImageCoordinates(0, 0, getMeasuredWidth(), getMeasuredHeight());
	}
	...
	public void setAvatar(TLObject obj) {
		...
		if (obj instanceof User) {
			objName = UserObject.getFirstName((User)obj);
		}
	...
	public void setProgress(float progress, boolean animate, boolean useSpring) {
		if (animate) {
			...
			if (useSpring) {
				...
				menuSpring.setSpring(new SpringForce(finalPos).setFinalPosition(finalPos).setStiffness(450f).setDampingRatio(SpringForce.DAMPING_RATIO_NO_BOUNCY));
