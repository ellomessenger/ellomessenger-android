...
public class FilterShaders {
	...
	private static final String sharpenVertexShaderCode = "attribute vec4 position;" + "attribute vec2 inputTexCoord;" + "varying vec2 texCoord;" +

			"uniform highp float inputWidth;" + "uniform highp float inputHeight;" + "varying vec2 leftTexCoord;" + "varying vec2 rightTexCoord;" + "varying vec2 topTexCoord;" + "varying vec2 bottomTexCoord;" +

			"void main() {" + "gl_Position = position;" + "texCoord = inputTexCoord;" + "highp vec2 widthStep = vec2(1.0 / inputWidth, 0.0);" + "highp vec2 heightStep = vec2(0.0, 1.0 / inputHeight);" + "leftTexCoord = inputTexCoord - widthStep;" + "rightTexCoord = inputTexCoord + widthStep;" + "topTexCoord = inputTexCoord + heightStep;" + "bottomTexCoord = inputTexCoord - heightStep;" + "}";

	private static final String sharpenFragmentShaderCode = "precision highp float;" + "varying vec2 texCoord;" + "varying vec2 leftTexCoord;" + "varying vec2 rightTexCoord;" + "varying vec2 topTexCoord;" + "varying vec2 bottomTexCoord;" + "uniform sampler2D sourceImage;" + "uniform float sharpen;" +

			"void main() {" + "vec4 result = texture2D(sourceImage, texCoord);" +
	...
	private static class ToneCurve {
		...
		private final float[] rgbCompositeCurve;
		private final float[] redCurve;
		private final float[] greenCurve;
		private final float[] blueCurve;
		private final int[] curveTexture = new int[1];
	...
	private final int[] rgbToHsvShaderProgram = new int[2];
	private final int[] rgbToHsvPositionHandle = new int[2];
	private final int[] rgbToHsvInputTexCoordHandle = new int[2];
	private final int[] rgbToHsvSourceImageHandle = new int[2];
	...
	private final int[] enhanceTextures = new int[2];
	private final int[] enhanceFrameBuffer = new int[1];
	private final int[] renderTexture = new int[4];
	private final int[] bitmapTextre = new int[1];
	private int[] renderFrameBuffer;
	private final int[] curveTextures = new int[1];
