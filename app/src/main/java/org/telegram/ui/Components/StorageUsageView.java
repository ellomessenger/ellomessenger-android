...
public class StorageUsageView extends FrameLayout {
	...
	private final Paint paintProgress2 = new Paint(Paint.ANTI_ALIAS_FLAG);
	...
	public StorageUsageView(Context context) {
		...
		legendLayout = new FrameLayout(context) {
			...
			protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
				...
				for (int i = 0; i < n; i++) {
					...
					getChildAt(i).layout(currentW, currentH, currentW + getChildAt(i).getMeasuredWidth(), currentH + getChildAt(i).getMeasuredHeight());
		...
		calculatingTextView.setTextColor(context.getColor(R.color.dark_gray));

		String threeDots = "...";
		String realThreeDots = "…";
		String calculatingString = context.getString(R.string.CalculatingSize).replace(realThreeDots, threeDots);
		int indexOfDots = calculatingString.indexOf(threeDots);
	...
	public void setStorageUsage(boolean calculating, long database, long totalSize, long totalDeviceFreeSize, long totalDeviceSize) {
		...
		if (calculating) {
			...
			if (totalSize > 0) {
				...
				textSettingsCell.setText(getContext().getString(R.string.ClearTelegramCache), false);
		...
		textSettingsCell.setTextColor(getContext().getColor(R.color.text));
	...
	public void invalidate() {
		...
		if (lastProgressColor != getContext().getColor(R.color.brand)) {
			lastProgressColor = getContext().getColor(R.color.brand);
	...
	private class ProgressView extends View {
		...
		protected void onDraw(Canvas canvas) {
			super.onDraw(canvas);
			int color = getContext().getColor(R.color.brand);
			...
			bgPaint.setColor(getContext().getColor(R.color.background));
