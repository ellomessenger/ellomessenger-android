...
public class AnimatedEmojiDrawable extends Drawable {
	...
	public static AnimatedEmojiDrawable make(int account, int cacheType, @NonNull TLRPC.Document document) {
		...
		if (drawable == null) {
			cache.put(document.id, drawable = new AnimatedEmojiDrawable(cacheType, document));
		}
		return drawable;
	}
	...
	public AnimatedEmojiDrawable(int cacheType, @NonNull TLRPC.Document document) {
		this.cacheType = cacheType;
		this.document = document;
		updateSize();
		this.initDocument();
	...
	private void initDocument() {
		...
		if (cacheType != 0) {
			imageReceiver.setUniqueKeyPrefix(cacheType + "_");
		}
	...
	public void draw(@NonNull Canvas canvas, boolean canTranslate) {
		if (imageReceiver != null) {
			imageReceiver.setImageCoordinates(getBounds());
	...
	private final AnimatedFloat placeholderAlpha = new AnimatedFloat(1f, this::invalidate, 0, 150, new LinearInterpolator());
		...
		public void draw(@NonNull Canvas canvas) {
			...
			if (drawables[1] != null && progress < 1) {
				...
				if (drawables[1] instanceof AnimatedEmojiDrawable) {
					...
					drawables[1].setBounds(bounds.centerX() - drawables[1].getIntrinsicWidth() / 2, bounds.centerY() - drawables[1].getIntrinsicHeight() / 2, bounds.centerX() + drawables[1].getIntrinsicWidth() / 2, bounds.centerY() + drawables[1].getIntrinsicHeight() / 2);
				}
				else { // left
					drawables[1].setBounds(bounds.left, bounds.centerY() - drawables[1].getIntrinsicHeight() / 2, bounds.left + drawables[1].getIntrinsicWidth(), bounds.centerY() + drawables[1].getIntrinsicHeight() / 2);
				}
			...
			if (drawables[0] != null) {
				...
				if (drawables[0] instanceof AnimatedEmojiDrawable) {
					...
					drawables[0].setBounds(bounds.centerX() - drawables[0].getIntrinsicWidth() / 2, bounds.centerY() - drawables[0].getIntrinsicHeight() / 2, bounds.centerX() + drawables[0].getIntrinsicWidth() / 2, bounds.centerY() + drawables[0].getIntrinsicHeight() / 2);
				}
				else { // left
					if (progress < 1) {
						float scale = overshootInterpolator.getInterpolation(progress);
					...
					drawables[0].setBounds(bounds.left, bounds.centerY() - drawables[0].getIntrinsicHeight() / 2, bounds.left + drawables[0].getIntrinsicWidth(), bounds.centerY() + drawables[0].getIntrinsicHeight() / 2);
				}
