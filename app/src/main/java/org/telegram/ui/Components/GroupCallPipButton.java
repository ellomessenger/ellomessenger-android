...
public class GroupCallPipButton extends FrameLayout implements NotificationCenter.NotificationCenterDelegate, StateListener {
	...
	protected void onDraw(@NonNull Canvas canvas) {
		...
		boolean showWaves = currentState.currentState != MUTE_BUTTON_STATE_MUTED_BY_ADMIN && currentState.currentState != MUTE_BUTTON_STATE_RECONNECT;
	...
	private void updateButtonState() {
		...
		if (voIPService != null && voIPService.groupCall != null) {
			...
			if (currentCallState == VoIPService.STATE_WAIT_INIT || currentCallState == VoIPService.STATE_WAIT_INIT_ACK || currentCallState == VoIPService.STATE_CREATING || currentCallState == VoIPService.STATE_RECONNECTING) {
				...
				var participant = voIPService.groupCall.participants.get(voIPService.getSelfId());
