...
public class AnimatedFileDrawable extends BitmapDrawable implements Animatable, BitmapsCache.Cacheable {
    ...
    private Runnable uiRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (stream != null && pendingRemoveLoading) {
                FileLoader.getInstance(currentAccount).removeLoadingVideo(stream.document, false, false);
            }
    ...
    public AnimatedFileDrawable makeCopy() {
        ...
        if (stream != null) {
            drawable = new AnimatedFileDrawable(path, false, streamFileSize, stream.document, stream.location, stream.getParentObject(), pendingSeekToUI, currentAccount, stream != null && stream.isPreview(), null);
        } else {
