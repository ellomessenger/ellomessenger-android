...
public class GroupCallFullscreenAdapter extends RecyclerListView.SelectionAdapter {
	...
	private final ArrayList<TLRPC.TLGroupCallParticipant> participants = new ArrayList<>();
	...
	public class GroupCallUserCell extends FrameLayout implements GroupCallStatusIcon.Callback {
		...
		public void setParticipant(ChatObject.VideoParticipant videoParticipant, TLRPC.TLGroupCallParticipant participant) {
			...
			if (peerId > 0) {
				...
				avatarImageView.imageReceiver.setCurrentAccount(currentAccount);
	...
	public void update(boolean animated, RecyclerListView listView) {
		...
		if (animated) {
			ArrayList<TLRPC.TLGroupCallParticipant> oldParticipants = new ArrayList<>(participants);
			ArrayList<ChatObject.VideoParticipant> oldVideoParticipants = new ArrayList<>(videoParticipants);

			participants.clear();
			if (!TLRPCExtensions.getRtmpStream(groupCall.call)) {
			...
			DiffUtil.calculateDiff(new DiffUtil.Callback() {
				...
				public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
					...
					TLRPC.TLGroupCallParticipant oldParticipant;
					TLRPC.TLGroupCallParticipant newParticipant;
