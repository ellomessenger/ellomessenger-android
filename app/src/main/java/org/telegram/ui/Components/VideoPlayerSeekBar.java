...
public class VideoPlayerSeekBar {
	...
	private StaticLayout makeStaticLayout(CharSequence text, int width) {
		...
		return StaticLayout.Builder.obtain(text, 0, text.length(), timestampLabelPaint, width).setMaxLines(1).setAlignment(Layout.Alignment.ALIGN_CENTER).setEllipsize(TextUtils.TruncateAt.END).setEllipsizedWidth(Math.min(AndroidUtilities.dp(400), width)).build();
	}
