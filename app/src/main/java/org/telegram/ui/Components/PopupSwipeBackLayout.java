...
public class PopupSwipeBackLayout extends FrameLayout {
	...
	private final ArrayList<OnSwipeBackProgressListener> onSwipeBackProgressListeners = new ArrayList<>();
	...
	protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
		...
		if (i != 0) {
			if (foregroundColor == 0) {
				foregroundPaint.setColor(Theme.getColor(Theme.key_actionBarDefaultSubmenuBackground));
			}
			else {
...
//		}
	...
	public void setNewForegroundHeight(int index, int height, boolean animated) {
		...
		if (animated) {
			...
			ValueAnimator animator = ValueAnimator.ofFloat(fromH, (float)height).setDuration(240);
