...
public class MessageContainsEmojiButton extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
	...
	private final Rect emojiDrawableBounds = new Rect();
	...
	private static class BoldAndAccent extends CharacterStyle {
		@Override
	...
	public MessageContainsEmojiButton(int currentAccount, Context context, @NonNull List<TLRPC.InputStickerSet> inputStickerSets, int type) {
		super(context);
		this.currentAccount = currentAccount;
		this.type = type;

		setBackground(Theme.createRadSelectorDrawable(Theme.getColor(Theme.key_listSelector), 0, 6));

		textPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
		textPaint.setTextSize(AndroidUtilities.dp(13));
		textPaint.setColor(Theme.getColor(Theme.key_actionBarDefaultSubmenuItem));
		...
		if (inputStickerSets.size() > 1) {
			...
			String string;
			if (type == EMOJI_TYPE) {
				string = context.getString(R.string.MessageContainsEmojiPack);
			}
			else {
				string = context.getString(R.string.MessageContainsReactionsPack);
			}
			...
			if (parts.length <= 1) {
				...
				if (inputStickerSet != null) {
					...
					if (stickerSet != null && stickerSet.set != null) {
						...
						if (document == null && stickerSet.documents != null && !stickerSet.documents.isEmpty()) {
				...
				if (stickerPackName != null && document != null) {
					...
					mainText = new SpannableStringBuilder().append(parts[0]).append(emoji).append(' ').append(stickerPack).append(parts[1]);
					loadT = 1f;
	...
	public void didReceivedNotification(int id, int account, Object... args) {
		if (id == NotificationCenter.groupStickersDidLoad) {
			if (inputStickerSet != null) {
				...
				secondPartText = new SpannableStringBuilder().append(margin).append(emoji).append(' ').append(stickerPack).append(endText);
