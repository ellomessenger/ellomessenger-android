...
public class DefaultThemesPreviewCell extends LinearLayout {
	...
	public DefaultThemesPreviewCell(Context context, BaseFragment parentFragment, int type) {
		...
		adapter = new ChatThemeBottomSheet.Adapter(parentFragment.getCurrentAccount(), currentType == ThemeActivity.THEME_TYPE_BASIC ? ThemeSmallPreviewView.TYPE_DEFAULT : ThemeSmallPreviewView.TYPE_GRID);
		...
		recyclerView.setOnItemClickListener((view, position) -> {
			...
			if (info == null) {
				TLRPC.TLTheme theme = chatTheme.chatTheme.getTlTheme(themeIndex);
				int settingsIndex = chatTheme.chatTheme.getSettingsIndex(themeIndex);
				TLRPC.TLThemeSettings settings = theme.settings.get(settingsIndex);
				...
				public void onClick(View view) {
					...
					Runnable then = () -> AndroidUtilities.runOnUIThread(() -> {
						...
						valueAnimator.addUpdateListener(valueAnimator2 -> {
							int iconColor = ColorUtils.blendARGB(iconOldColor, iconNewColor, (float)valueAnimator2.getAnimatedValue());
						...
						if (window != null) {
							...
							navBarAnimator.addUpdateListener(valueAnimator1 -> {
								float t = Math.max(0, Math.min(1, ((float)valueAnimator1.getAnimatedValue() * fullDuration - startDelay) / duration));
	...
	private void updateSelectedPosition() {
		...
		selectedPosition = -1;
		for (int i = 0; i < adapter.items.size(); i++) {
			TLRPC.TLTheme theme = adapter.items.get(i).chatTheme.getTlTheme(themeIndex);
...
//        }
	...
	public void updateColors() {
		if (currentType == ThemeActivity.THEME_TYPE_BASIC) {
			...
			browseThemesCell.setBackground(Theme.createSelectorWithBackgroundDrawable(getContext().getColor(R.color.background), Theme.getColor(Theme.key_listSelector)));
