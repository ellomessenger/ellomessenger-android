...
public class PopupNotificationActivity extends Activity implements NotificationCenter.NotificationCenterDelegate {
	...
	private final ArrayList<ViewGroup> textViews = new ArrayList<>();
	private final ArrayList<ViewGroup> imageViews = new ArrayList<>();
	private final ArrayList<ViewGroup> audioViews = new ArrayList<>();
	...
	private final MessageObject[] setMessageObjects = new MessageObject[3];
	...
	private final ArrayList<MessageObject> popupMessages = new ArrayList<>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		popupContainer.setBackgroundResource(R.color.background);
		...
		chatActivityEnterView = new ChatActivityEnterView(this, contentView, null, false, false);
		...
		chatActivityEnterView.setDelegate(new ChatActivityEnterView.ChatActivityEnterViewDelegate() {
			@Override
			public void onBotCommandSelected(@NonNull String command) {
				// unused
			}
		...
		wakeLock = pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP, "ello:screen");
		...
		if ((messageObject.type == 1 || messageObject.type == 4) && !messageObject.isSecretMedia()) {
			...
			if (messageObject.type == 1) {
				...
				if (MessagesController.getInstance(messageObject.currentAccount).mapProvider == MessagesController.MAP_PROVIDER_ELLO) {
					...
					String currentUrl = AndroidUtilities.formatMapUrl(messageObject.currentAccount, lat, lon, 100, 100, true, 15, -1);
	...
	private void handleIntent(Intent intent) {
		...
		if (km.inKeyguardRestrictedInputMode() || !ApplicationLoader.isScreenOn) {
			getWindow().addFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND | WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED | WindowManager.LayoutParams.FLAG_TURN_SCREEN_ON | WindowManager.LayoutParams.FLAG_FORCE_NOT_FULLSCREEN);
		}
		else {
			getWindow().addFlags(WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED | WindowManager.LayoutParams.FLAG_TURN_SCREEN_ON | WindowManager.LayoutParams.FLAG_FORCE_NOT_FULLSCREEN);
	...
	private void updateSubtitle() {
		...
		if (currentUser != null && currentUser.id == 777000) {
			...
			if (printString == null || printString.length() == 0) {
				...
				User user = MessagesController.getInstance(currentMessageObject.currentAccount).getUser(currentUser.id);
				if (user != null) {
					currentUser = user;
				}
