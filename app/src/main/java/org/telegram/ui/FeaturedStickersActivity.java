...
public class FeaturedStickersActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
	...
	private List<Long> unreadStickers = null;
	...
	public View createView(@NonNull Context context) {
		...
		actionBar.setTitle(context.getString(R.string.FeaturedStickers));
	...
	private void updateRows() {
		rowCount = 0;
		List<TLRPC.StickerSetCovered> stickerSets = MediaDataController.getInstance(currentAccount).getFeaturedStickerSets();
	...
	private class ListAdapter extends RecyclerListView.SelectionAdapter {
		...
		public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
			if (getItemViewType(position) == 0) {
				List<TLRPC.StickerSetCovered> arrayList = MediaDataController.getInstance(currentAccount).getFeaturedStickerSets();
			...
			switch (viewType) {
				case 0:
					...
					((FeaturedStickerSetCell)view).setAddOnClickListener(v -> {
						...
						MediaDataController.getInstance(currentAccount).toggleStickerSet(mContext, pack, 2, FeaturedStickersActivity.this, false, false);
