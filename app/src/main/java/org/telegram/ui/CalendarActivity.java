...
public class CalendarActivity extends BaseFragment {
	...
	private final Paint selectOutlinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
	...
	int lastId;
	int minMontYear;
	private final int photosVideosTypeFilter;
	...
	private void updateColors() {
		activeTextPaint.setColor(Color.WHITE);
		textPaint.setColor(getContext().getColor(R.color.text));
		textPaint2.setColor(getContext().getColor(R.color.text));
	}
	...
	private class MonthView extends FrameLayout {
			...
			gestureDetector = new GestureDetectorCompat(context, new GestureDetector.SimpleOnGestureListener() {
				...
				public void onLongPress(MotionEvent e) {
					...
					if (periodDay != null) {
						...
						ActionBarPopupWindow.ActionBarPopupWindowLayout previewMenu = new ActionBarPopupWindow.ActionBarPopupWindowLayout(getParentActivity(), R.drawable.popup_fixed_alert);
		...
		private final SparseArray<ValueAnimator> rowAnimators = new SparseArray<>();
		private final SparseArray<RowAnimationValue> rowSelectionPos = new SparseArray<>();
				...
				if (nowTime < startMonthTime + (i + 1) * 86400 || (minDate > 0 && minDate > startMonthTime + (i + 2) * 86400)) {
					...
					if (imagesByDays.get(i) != null) {
						...
						if (day.selectStartEndProgress >= 0.01f) {
							selectPaint.setColor(getContext().getColor(R.color.background));
						...
						imagesByDays.get(i).setImageCoordinates(cx - (AndroidUtilities.dp(44) - pad) / 2f, cy - (AndroidUtilities.dp(44) - pad) / 2f, AndroidUtilities.dp(44) - pad, AndroidUtilities.dp(44) - pad);
