...
public class MessageSeenView extends FrameLayout {
	...
	public MessageSeenView(@NonNull Context context, int currentAccount, MessageObject messageObject, TLRPC.Chat chat) {
		...
		flickerLoadingView.setColors(ResourcesCompat.getColor(context.getResources(), R.color.background, null), ResourcesCompat.getColor(context.getResources(), R.color.light_background, null), 0);
		...
		ConnectionsManager.getInstance(currentAccount).sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
			if (error == null) {
				...
				HashMap<Long, User> usersLocal = new HashMap<>();
