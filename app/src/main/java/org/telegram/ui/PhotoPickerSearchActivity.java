...
public class PhotoPickerSearchActivity extends BaseFragment {
	...
	private final PhotoPickerActivity imagesSearch;
	private final PhotoPickerActivity gifsSearch;
	...
	public PhotoPickerSearchActivity(Map<Object, Object> selectedPhotos, List<Object> selectedPhotosOrder, int selectPhotoType, boolean allowCaption, ChatActivity chatActivity) {
		super();

		imagesSearch = new PhotoPickerActivity(0, null, selectedPhotos, selectedPhotosOrder, selectPhotoType, allowCaption, chatActivity, false, false);
		gifsSearch = new PhotoPickerActivity(1, null, selectedPhotos, selectedPhotosOrder, selectPhotoType, allowCaption, chatActivity, false, false);
	}
	...
	public View createView(@NonNull Context context) {
		...
		searchItem = menu.addItem(search_button, R.drawable.ic_search_menu).setIsSearchField(true).setActionBarMenuItemSearchListener(new ActionBarMenuItem.ActionBarMenuItemSearchListener() {
			@Override
...
//        sizeNotifierFrameLayout.addView(imagesSearch.shadow, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 3, Gravity.BOTTOM | Gravity.LEFT, 0, 0, 0, 48));
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && AndroidUtilities.computePerceivedBrightness(statusBarBackground) >= 0.721f) {
			fragmentView.setSystemUiVisibility(fragmentView.getSystemUiVisibility() | View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
		}
