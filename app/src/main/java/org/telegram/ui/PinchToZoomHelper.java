...
public class PinchToZoomHelper {
	...
	public void startZoom(View child, ImageReceiver image, @Nullable MessageObject messageObject) {
		this.child = child;
		...
		if (messageObject != null && messageObject.isVideo() && MediaController.getInstance().isPlayingMessage(messageObject)) {
			...
			if (callback != null) {
				...
				overlayView.backupImageView.imageReceiver.setRoundRadius(image.getRoundRadius());
			}
			...
			this.childImage.setImageCoordinates(imageX, imageY, imageWidth, imageHeight);
	...
	private void setFullImage(@Nullable MessageObject messageObject) {
		...
		if (imageLocation != null) {
			boolean cacheOnly = messageObject.isWebpage();
			Object parentObject;
			parentObject = messageObject;

			fullImage.setImage(imageLocation, null, null, null, null, size[0], null, parentObject, cacheOnly ? 1 : 0);
			fullImage.setCrossfadeAlpha((byte)2);
		}
	...
	public void clear() {
		...
		if (overlayView != null && overlayView.getParent() != null) {
			...
			overlayView.backupImageView.imageReceiver.clearImage();
	...
	protected boolean zoomEnabled(View child, @Nullable ImageReceiver receiver) {
		if (receiver == null) {
			return false;
		}
	...
	private class ZoomOverlayView extends FrameLayout {
		...
		private void drawImage(Canvas canvas) {
			...
			canvas.save();
			float s = pinchScale * finishProgress + 1.0f - finishProgress;
			...
			if (imageHeight != fullImageHeight || imageWidth != fullImageWidth) {
				...
				if (childImage != null) {
					childImage.setImageCoordinates(x, y, imageWidth + horizontalPadding * p * 2, imageHeight + verticalPadding * p * 2);
				}
			}
	...
	public interface Callback {
		...
		default void onZoomStarted(@Nullable MessageObject messageObject) {

		}

		default void onZoomFinished(@Nullable MessageObject messageObject) {
	...
	public boolean checkPinchToZoom(MotionEvent ev, View child, @Nullable ImageReceiver image, @Nullable MessageObject messageObject) {
