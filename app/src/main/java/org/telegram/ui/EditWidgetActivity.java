...
public class EditWidgetActivity extends BaseFragment {
	...
	public class WidgetPreviewCell extends FrameLayout {
		...
		private final ViewGroup[] cells = new ViewGroup[2];
		...
		public WidgetPreviewCell(Context context) {
			...
			chatActionCell.setCustomText(context.getString(R.string.WidgetPreview));
					...
					if (DialogObject.isUserDialog(dialog.id)) {
						...
						if (user != null) {
							if (UserObject.isUserSelf(user)) {
								name = getContext().getString(R.string.SavedMessages);
							}
							else if (UserObject.isReplyUser(user)) {
								name = getContext().getString(R.string.RepliesTitle);
							}
						...
						if (message.messageOwner instanceof TLRPC.TL_messageService) {
							...
							if (chat != null && chat.id > 0 && fromChat == null && (!ChatObject.isChannel(chat) || ChatObject.isMegagroup(chat))) {
								if (message.isOutOwner()) {
									messageNameString = getContext().getString(R.string.FromYou);
								}
								...
								if (message.caption != null) {
									...
									try {
										stringBuilder.setSpan(new ForegroundColorSpanThemable(getContext().getColor(R.color.darker_brand)), messageNameString.length() + 2, stringBuilder.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
									}
									catch (Exception e) {
										FileLog.e(e);
									}
								...
								try {
									stringBuilder.setSpan(new ForegroundColorSpanThemable(getContext().getColor(R.color.darker_brand)), 0, messageNameString.length() + 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
								}
								catch (Exception e) {
									FileLog.e(e);
								}
	...
	public View createView(@NonNull Context context) {
		...
		if (widgetType == TYPE_CHATS) {
			actionBar.setTitle(context.getString(R.string.WidgetChats));
		}
		else {
			actionBar.setTitle(context.getString(R.string.WidgetShortcuts));
		}
		ActionBarMenu menu = actionBar.createMenu();
		menu.addItem(done_item, context.getString(R.string.Done).toUpperCase());
		...
		listView.setOnItemLongClickListener(new RecyclerListView.OnItemLongClickListenerExtended() {
			...
			public boolean onItemClick(@NonNull View view, int position, float x, float y) {
				...
				if (!rect.contains((int)x, (int)y)) {
					...
					CharSequence[] items = new CharSequence[]{context.getString(R.string.Delete)};
			...
			switch (holder.getItemViewType()) {
				case 0: {
					...
					if (position == infoRow) {
						...
						if (widgetType == TYPE_CHATS) {
							builder.append(mContext.getString(R.string.EditWidgetChatsInfo));
						}
						else if (widgetType == TYPE_CONTACTS) {
							builder.append(mContext.getString(R.string.EditWidgetContactsInfo));
						}
						if (SharedConfig.passcodeHash.length() > 0) {
							builder.append("\n\n").append(AndroidUtilities.replaceTags(mContext.getString(R.string.WidgetPasscode2)));
						}
				...
				case 1: {
					...
					cell.setTextAndIcon(mContext.getString(R.string.SelectChats), combinedDrawable, chatsStartRow != -1);
