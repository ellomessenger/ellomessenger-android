...
public class TwoStepVerificationActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
	...
	public interface TwoStepVerificationActivityDelegate {
		void didEnterPassword(@Nullable TLRPC.InputCheckPasswordSRP password);
	}
	...
	public void setPassword(TLRPC.TLAccountPassword password) {
		currentPassword = password;
		passwordEntered = false;
	}

	public void setCurrentPasswordParams(TLRPC.TLAccountPassword password, byte[] passwordHash, long secretId, byte[] secret) {
		currentPassword = password;
		currentPasswordHash = passwordHash;
		currentSecret = secret;
		currentSecretId = secretId;
		passwordEntered = currentPasswordHash != null && currentPasswordHash.length > 0 || !currentPassword.hasPassword;
	}

	@Override
	public boolean onFragmentCreate() {
		super.onFragmentCreate();
		if (currentPassword == null || currentPassword.currentAlgo == null || currentPasswordHash == null || currentPasswordHash.length == 0) {
	...
	public View createView(@NonNull Context context) {
		...
		passwordEditText.setTypeface(Theme.TYPEFACE_DEFAULT);
		...
		listView.setOnItemClickListener((view, position) -> {
			if (position == setPasswordRow || position == changePasswordRow) {
				...
				if (currentPassword.hasSecureValues) {
	...
	private void cancelPasswordReset() {
		...
		builder.setPositiveButton(LocaleController.getString("CancelPasswordResetYes", R.string.CancelPasswordResetYes), (dialog, which) -> {
			var req = new TLRPC.TLAccountDeclinePasswordReset();
			getConnectionsManager().sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
				if (response instanceof TLRPC.TLBoolTrue) {
					currentPassword.pendingResetDate = 0;
					updateBottomButton();
				}
			}));
		});
	...
	private void resetPassword() {
		...
		var req = new TLRPC.TLAccountResetPassword();
		getConnectionsManager().sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
			needHideProgress();
			if (response instanceof TLRPC.TLAccountResetPasswordOk) {
				AlertDialog.Builder builder = new AlertDialog.Builder(getParentActivity());
			...
			else if (response instanceof TLRPC.TLAccountResetPasswordRequestedWait res) {
				currentPassword.pendingResetDate = res.untilDate;
				updateBottomButton();
			}
			else if (response instanceof TLRPC.TLAccountResetPasswordFailedWait res) {
				int time = res.retryDate - getConnectionsManager().getCurrentTime();
				String timeString;
				if (time > 24 * 60 * 60) {
	...
	private void updateBottomButton() {
		...
		if (currentPassword.pendingResetDate == 0 || getConnectionsManager().getCurrentTime() > currentPassword.pendingResetDate) {
			if (resetWaitView.getVisibility() != View.GONE) {
				resetWaitView.setVisibility(View.GONE);
			}
			if (currentPassword.pendingResetDate == 0) {
			...
			int t = Math.max(1, currentPassword.pendingResetDate - getConnectionsManager().getCurrentTime());
			String time;
			if (t > 24 * 60 * 60) {
	...
	private void onPasswordForgot() {
		if (currentPassword.pendingResetDate == 0 && currentPassword.hasRecovery) {
			needShowProgress(true);
			var req = new TLRPC.TLAuthRequestPasswordRecovery();
			ConnectionsManager.getInstance(currentAccount).sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
				needHideProgress();
				if (error == null) {
					final var res = (TLRPC.TLAuthPasswordRecovery)response;
			...
			if (currentPassword.pendingResetDate != 0) {
				if (getConnectionsManager().getCurrentTime() > currentPassword.pendingResetDate) {
					AlertDialog.Builder builder = new AlertDialog.Builder(getParentActivity());
	...
	public void setCurrentPasswordInfo(byte[] hash, TLRPC.TLAccountPassword password) {
		if (hash != null) {
	...
	public static boolean canHandleCurrentPassword(TLRPC.TLAccountPassword password, boolean login) {
		if (login) {
			return !(password.currentAlgo instanceof TLRPC.TLPasswordKdfAlgoUnknown);
		}
		else {
			return !(password.newAlgo instanceof TLRPC.TLPasswordKdfAlgoUnknown) && !(password.currentAlgo instanceof TLRPC.TLPasswordKdfAlgoUnknown) && !(password.newSecureAlgo instanceof TLRPC.TLSecurePasswordKdfAlgoUnknown);
		}
	}

	public static void initPasswordNewAlgo(TLRPC.TLAccountPassword password) {
		if (password.newAlgo instanceof TLRPC.TLPasswordKdfAlgoModPow algo) {
		...
		if (password.newSecureAlgo instanceof TLRPC.TLSecurePasswordKdfAlgoPBKDF2 algo) {
	...
	private void loadPasswordInfo(boolean first, final boolean silent) {
		...
		var req = new TLRPC.TLAccountGetPassword();
		ConnectionsManager.getInstance(currentAccount).sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
			if (error == null) {
				loading = false;
		...
		if (fragmentView != null) {
			if (loading || passwordEntered) {
				...
				if (passwordEditText != null) {
					...
					fragmentView.setBackgroundColor(getContext().getColor(R.color.background));
					fragmentView.setTag(Theme.key_windowBackgroundWhite);
	...
	private void clearPassword() {
		...
		final TLRPC.TLAccountUpdatePasswordSettings req = new TLRPC.TLAccountUpdatePasswordSettings();
		if (currentPasswordHash == null || currentPasswordHash.length == 0) {
			req.password = new TLRPC.TLInputCheckPasswordEmpty();
		}
		req.newSettings = new TLRPC.TLAccountPasswordInputSettings();

		UserConfig.getInstance(currentAccount).resetSavedPassword();
		currentSecret = null;
		req.newSettings.flags = 3;
		req.newSettings.hint = "";
		req.newSettings.newPasswordHash = new byte[0];
		req.newSettings.newAlgo = new TLRPC.TLPasswordKdfAlgoUnknown();
		req.newSettings.email = "";

		needShowProgress();
		Utilities.globalQueue.postRunnable(() -> {
			if (req.password == null) {
				if (currentPassword.currentAlgo == null) {
					var getPasswordReq = new TLRPC.TLAccountGetPassword();
	...
	@Nullable
	public TLRPC.TLInputCheckPasswordSRP getNewSrpPassword() {
	...
	private boolean checkSecretValues(byte[] passwordBytes, TLRPC.TLAccountPasswordSettings passwordSettings) {
		if (passwordSettings.secureSettings != null) {
			currentSecret = passwordSettings.secureSettings.secureSecret;
			byte[] passwordHash;
			if (passwordSettings.secureSettings.secureAlgo instanceof TLRPC.TLSecurePasswordKdfAlgoPBKDF2 algo) {
				passwordHash = Utilities.computePBKDF2(passwordBytes, algo.salt);
			}
			else if (passwordSettings.secureSettings.secureAlgo instanceof TLRPC.TLSecurePasswordKdfAlgoSHA512 algo) {
				passwordHash = Utilities.computeSHA512(algo.salt, passwordBytes, algo.salt);
			}
			else {
				return false;
			}
			currentSecretId = passwordSettings.secureSettings.secureSecretId;
			...
			if (!PassportActivity.checkSecret(passwordSettings.secureSettings.secureSecret, passwordSettings.secureSettings.secureSecretId)) {
				TLRPC.TLAccountUpdatePasswordSettings req = new TLRPC.TLAccountUpdatePasswordSettings();
				req.password = getNewSrpPassword();
				req.newSettings = new TLRPC.TLAccountPasswordInputSettings();
				req.newSettings.newSecureSettings = new TLRPC.TLSecureSecretSettings();
				req.newSettings.newSecureSettings.secureSecret = new byte[0];
				req.newSettings.newSecureSettings.secureAlgo = new TLRPC.TLSecurePasswordKdfAlgoUnknown();
				req.newSettings.newSecureSettings.secureSecretId = 0;
				req.newSettings.flags |= 4;
		...
		if (!passwordEntered) {
			...
			Utilities.globalQueue.postRunnable(() -> {
				...
				RequestDelegate requestDelegate = (response, error) -> {
					if (error == null) {
						Utilities.globalQueue.postRunnable(() -> {
							boolean secretOk = checkSecretValues(oldPasswordBytes, (TLRPC.TLAccountPasswordSettings)response);
							AndroidUtilities.runOnUIThread(() -> {
								...
								if (secretOk) {
									...
									if (delegate != null) {
										...
										if (!TextUtils.isEmpty(currentPassword.emailUnconfirmedPattern)) {
