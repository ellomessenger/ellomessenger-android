...
public final class FloatingToolbar {
	...
	private final Comparator<MenuItem> mMenuItemComparator = Comparator.comparingInt(MenuItem::getOrder);
	...
	private final class FloatingToolbarPopup {
		...
		private ImageButton createOverflowButton() {
			...
			if (currentStyle == STYLE_DIALOG) {
				color = mContext.getColor(R.color.text);
				overflowButton.setBackgroundDrawable(Theme.createSelectorDrawable(mContext.getColor(R.color.light_background), 1));
			}
	...
	private ViewGroup createContentContainer(Context context) {
		...
		if (currentStyle == STYLE_DIALOG) {
			shape.setColor(context.getColor(R.color.background));
		}
		else if (currentStyle == STYLE_BLACK) {
	...
	private static AnimatorSet createEnterAnimation(View view) {
		...
		animation.playTogether(ObjectAnimator.ofFloat(view, View.ALPHA, 0, 1).setDuration(150));
		return animation;
	}
