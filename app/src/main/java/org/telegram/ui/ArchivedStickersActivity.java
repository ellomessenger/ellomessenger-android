...
public class ArchivedStickersActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
		...
		listView.setOnItemClickListener((view, position) -> {
			if (position >= stickersStartRow && position < stickersEndRow && getParentActivity() != null) {
				...
				if (stickerSet.set.id != 0) {
					...
					// stickerSetShortName.accessHash = stickerSet.set.accessHash;
	...
	private void getStickers() {
		...
		var req = new TLRPC.TLMessagesGetArchivedStickers();
		req.offsetId = sets.isEmpty() ? 0 : sets.get(sets.size() - 1).set.id;
		req.limit = 15;
		...
		int reqId = getConnectionsManager().sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
			if (error == null) {
				processResponse((TLRPC.TLMessagesArchivedStickers)response);
			}
		}));
		getConnectionsManager().bindRequestToGuid(reqId, classGuid);
	}

	private void processResponse(TLRPC.TLMessagesArchivedStickers res) {
		if (!isInTransition) {
		...
		if (id == NotificationCenter.needAddArchivedStickers) {
			...
			if (listView != null) {
				for (int i = 0, size = listView.getChildCount(); i < size; i++) {
					...
					if (view instanceof ArchivedStickerSetCell cell) {
