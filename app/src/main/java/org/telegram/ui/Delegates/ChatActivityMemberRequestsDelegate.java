...
public class ChatActivityMemberRequestsDelegate {
	...
	public View getView() {
		if (root == null) {
			...
			requestsCountTextView.setTypeface(Theme.TYPEFACE_BOLD);
			...
			ImageView closeView = new ImageView(fragment.getParentActivity());
			...
			closeView.setContentDescription(fragment.getParentActivity().getString(R.string.Close));
			...
			if (chatInfo != null) {
				setPendingRequests(chatInfo.requestsPending, chatInfo.recentRequesters, false);
			}
		}
		return root;
	}
	...
	public void setChatInfo(@Nullable TLRPC.ChatFull chatInfo, boolean animated) {
		...
		if (chatInfo != null) {
			setPendingRequests(chatInfo.requestsPending, chatInfo.recentRequesters, animated);
		}
	}
		...
		if (pendingRequestsCount != count) {
			...
			if (recentRequestersIdList != null && !recentRequestersIdList.isEmpty()) {
				...
				for (int i = 0; i < usersCount; ++i) {
					User user = fragment.getMessagesController().getUser(recentRequestersIdList.get(i));
					if (user != null) {
