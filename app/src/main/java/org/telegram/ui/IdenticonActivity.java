...
public class IdenticonActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
	...
	public View createView(@NonNull Context context) {
		...
		codeTextView.setTypeface(Theme.TYPEFACE_MONOSPACE);
		...
		if (encryptedChat != null) {
			...
			if (encryptedChat.keyHash.length > 16) {
				String hex = Utilities.bytesToHex(encryptedChat.keyHash);
				for (int a = 0; a < 32; a++) {
					if (a != 0) {
						if (a % 8 == 0) {
							hash.append('\n');
				...
				hash.append("\n");
				for (int a = 0; a < 5; a++) {
					int num = ((encryptedChat.keyHash[16 + a * 4] & 0x7f) << 24) | ((encryptedChat.keyHash[16 + a * 4 + 1] & 0xff) << 16) | ((encryptedChat.keyHash[16 + a * 4 + 2] & 0xff) << 8) | (encryptedChat.keyHash[16 + a * 4 + 3] & 0xff);
					if (a != 0) {
						emojis.append(" ");
					}
			...
			hash.clear();
			hash.append(AndroidUtilities.replaceTags(LocaleController.formatString("EncryptionKeyDescription", R.string.EncryptionKeyDescription, user.firstName, user.firstName)));
			final String url = "ello.team";
