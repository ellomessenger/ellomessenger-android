...
public class GridLayoutManagerFixed extends GridLayoutManager {
	private final ArrayList<View> additionalViews = new ArrayList<>(4);
	...
	protected void measureChild(View view, int otherDirParentSpecMode, boolean alreadyMeasured) {
		...
		final int wSpec = getChildMeasureSpec(availableSpaceInOther, otherDirParentSpecMode, horizontalInsets, lp.width, false);
		final int hSpec = getChildMeasureSpec(mOrientationHelper.getTotalSpace(), getHeightMode(), verticalInsets, lp.height, true);
	...
	void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LayoutState layoutState, LayoutChunkResult result) {
		...
		while (working) {
			...
			boolean fromOpositeSide = shouldLayoutChildFromOppositeSide(mSet[0]);
