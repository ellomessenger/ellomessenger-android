...
Connection::Connection(Datacenter *datacenter, ConnectionType type, int8_t num) : ConnectionSession(datacenter->instanceNum), ConnectionSocket(datacenter->instanceNum) {
    ...
    reconnectTimer = new Timer(datacenter->instanceNum, [&] {
        if (LOGS_ENABLED) DEBUG_D("Reconnect timer fired");
    ...
    while (buffer->hasRemaining()) {
        ...
        if (currentProtocolType == ProtocolTypeEF) {
            ...
            if ((fInt & (0x80000000)) != 0) {
                ConnectionsManager::getInstance(currentDatacenter->instanceNum).onConnectionQuickAckReceived(this, (int32_t) (fInt & (~(1 << 31))));
                continue;
            }
...
void Connection::connect() {
    ...
    uint8_t strategy = ConnectionsManager::getInstance(currentDatacenter->instanceNum).getIpStrategy();
    uint32_t ipv6;
...
void Connection::setHasUsefulData() {
    if (!usefulData) {
...
void Connection::sendData(NativeByteBuffer *buff, bool reportAck, bool encrypted) {
    ...
    uint32_t additionalPacketSize = 0;
    ...
    if (currentProtocolType == ProtocolTypeEF) {
        ...
        if (currentProtocolType == ProtocolTypeDD || currentProtocolType == ProtocolTypeTLS) {
            RAND_bytes((uint8_t *) &additionalPacketSize, 4);

            if (!encrypted) {
                additionalPacketSize = additionalPacketSize % 257;
            }
            else {
                additionalPacketSize = additionalPacketSize % 16;
            }

            packetLength += additionalPacketSize;
        }
        else {
            RAND_bytes((uint8_t *) &additionalPacketSize, 4);
            ...
            if (!encrypted) {
                additionalPacketSize = additionalPacketSize % 257;
                uint32_t additionalSize = additionalPacketSize % 4;

                if (additionalSize != 0) {
                    additionalPacketSize += (4 - additionalSize);
                }
            }
    ...
    if (additionalPacketSize > 0) {
        buffer2 = BuffersStorage::getInstance().getFreeBuffer(additionalPacketSize);
        RAND_bytes(buffer2->bytes(), additionalPacketSize);
    }
    else {
        buffer2 = nullptr;
    }
    ...
    if (currentProtocolType == ProtocolTypeEF) {
        if (packetLength < 0x7f) {
            ...
            buffer->writeInt32((int32_t) packetLength);
...
int8_t Connection::getConnectionNum() const {
    return connectionNum;
}

uint32_t Connection::getConnectionToken() const {
