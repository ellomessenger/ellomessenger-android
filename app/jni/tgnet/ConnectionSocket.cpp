    ...
    if (!proxyAddress->empty()) {
        ...
        if (continueCheckAddress) {
            if (inet_pton(AF_INET6, address.c_str(), &socketAddress6.sin6_addr.s6_addr) != 1) {
                ...
                if (LOGS_ENABLED) {
                    DEBUG_D("connection(%p) not ipv6 address %s", this, address.c_str());
                }
            }
            else {
                ipv6 = true;
            ...
            if (continueCheckAddress) {
                auto host = ConnectionsManager::getInstance(instanceNum).delegate->getHostByNameSync(address, instanceNum, this);
				...
				if (host.empty()) {
					...
					if (LOGS_ENABLED) DEBUG_D("connection(%p) resolved host %s address: %s", this, address.c_str(), host.c_str());
				}


				openConnection(host, port, secret, ipv6, networkType);
	...
	ConnectionsManager::getInstance(instanceNum).scheduleTask([&, host, ip, ipv6] {
		if (waitingForHostResolve == host) {
			...
			if (ip.empty() || inet_pton(AF_INET, ip.c_str(), &socketAddress.sin_addr.s_addr) != 1) {
				if (LOGS_ENABLED) {
					DEBUG_E("connection(%p) can't resolve host %s address via delegate", this, host.c_str());
				}
			...
			if (LOGS_ENABLED) {
				DEBUG_D("connection(%p) resolved host %s address %s via delegate", this, host.c_str(), ip.c_str());
			}
