...
std::string messengerEndpoint;
uint32_t messengerPort = 0;
...
void ConnectionsManager::scheduleTask(const std::function<void()> &task) {
    pthread_mutex_lock(&mutex);
...
int32_t ConnectionsManager::getTimeDifference() const {
...
bool ConnectionsManager::isNetworkAvailable() const {
            ...
            if (!connection->isSuspended() && (proxyAddress.empty() || connection->hasTlsHashMismatch())) {
                ...
                if (disconnectTimeoutAmount >= maxTimeout) {
                    if (!connection->hasUsefulData()) {
                        ...
                        if (LOGS_ENABLED) DEBUG_D("connection has useful data, don't request anything");
                    }
    ...
    if (type == ConnectionTypeGeneric || type == ConnectionTypeTemp || type == ConnectionTypeGenericMedia) {
        ...
        for (auto &runningRequest: runningRequests) {
            ...
            if ((connectionType == type && connectionNum == num) || request->connectionToken == token) {
                if (LOGS_ENABLED) DEBUG_E("connection(%p) request (msg_id: %ld)", connection, request->messageId);
...
TLObject *ConnectionsManager::TLdeserialize(TLObject *request, uint32_t bytes, NativeByteBuffer *data) const {
    bool error = false;
            ...
            for (auto iter = runningRequests.begin(); iter != runningRequests.end(); iter++) {
                ...
                if (request->onCompleteRequestCallback != nullptr) {
                    ...
                    if (error != nullptr) {
                        ...
                        static std::string userDeactivated = "USER_DEACTIVATED";
                        ...
                        else if (error->error_code == 403 && error->error_message.find(userDeactivated) != std::string::npos) {
                            discardResponse = true;
                            request->minStartTime = (int32_t) (getCurrentTimeMonotonicMillis() / 1000 + 1);
                            request->startTime = 0;
...
uint8_t ConnectionsManager::getIpStrategy() const {
    return ipStrategy;
}
...
jfieldID getBuildConfigField(JNIEnv *env, const char *name, const char *type) {
    auto build_config_class = env->FindClass("org/telegram/messenger/BuildConfig");
    return env->GetStaticFieldID(build_config_class, name, type);
}

std::string getBuildConfigStringField(JNIEnv *env, const char *name) {
    auto key_id = getBuildConfigField(env, name, "Ljava/lang/String;");
    auto key = (jstring) env->GetStaticObjectField(env->FindClass("org/telegram/messenger/BuildConfig"), key_id);
...
std::string getMessengerEndpointString(JNIEnv *env) {
    return getBuildConfigStringField(env, "MESSENGER_ENDPOINT");
}

uint32_t getMessengerPort(JNIEnv *env) {
    jfieldID messenger_port_id = getBuildConfigField(env, "MESSENGER_PORT", "I");
    return (uint32_t) env->GetStaticIntField(env->FindClass("org/telegram/messenger/BuildConfig"), messenger_port_id);
}
...
void ConnectionsManager::initDatacenters() {
    ...
    if (datacenters.find(1) == datacenters.end()) {
        ...
        datacenter->addAddressAndPort(messengerEndpoint, messengerPort, 0, "");
        ...
        if (forceThisRequest || (abs(currentTime - request->startTime) > maxTimeout &&
            ...
            if (!request->failedBySalt) {
                if (request->connectionType & ConnectionTypeDownload) {
                    auto retryMax = request->getMaxRetryCount();
...
void ConnectionsManager::init(uint32_t version, int32_t layer, int32_t apiId, std::string deviceModel, std::string systemVersion, std::string appVersion, std::string langCode, std::string systemLangCode, std::string configPath, std::string logPath, std::string regId, std::string cFingerpting, std::string installerId, std::string packageId, int32_t timezoneOffset, int64_t userId, bool isPaused, bool enablePushConnection, bool hasNetwork, int32_t networkType) {
    ...
    messengerEndpoint = getMessengerEndpointString(env);
    messengerPort = getMessengerPort(env);
    datacenterPublicKey = getBuildConfigStringField(env, "DATACENTER_PUBLIC_KEY");
    serverPublicKey = getBuildConfigStringField(env, "SERVER_PUBLIC_KEY");
    serverPublicKeyFingerprint = strtoull(getBuildConfigStringField(env, "SERVER_PUBLIC_KEY_FINGERPRINT").c_str(), nullptr, 16);
    ...
    scheduleTask([&, partial] {
        if (lastMonotonicPauseTime != 0) {
            ...
            if (systemDiff < 0 || abs(systemDiff - diff) > 2) {
                timeDifference -= (int32_t) (systemDiff - diff);
            }
        }
